...
- A fourth registry, **`STORYLINE_VARIANTS`**, lets the same timeline play
  ...
  picks still flow through the existing rollers (`rollTrainerTeam`,
  `rollWildEncounter`), which read `sm.settings.enabledGens` and the row's
  grade weights, so a variant changes the *story around* the fight, never
  the foe roster math.
...
- `STORY_MODE_FLOW.md` §17 documents the architecture, the common-case
  ...
  rolls bound by enabled gens and row grade weights).
