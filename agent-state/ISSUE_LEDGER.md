...
- [ISSUE-019] [P1] Late-game enemy weak-mon floor + endgame gold over-accumulation (2026-06 balance pass) — IMPLEMENTED — `_storyFillerGradeFloorForRow` (balance)
- [ISSUE-020] [P1] Villain-boss Master Ball grant has no fire-once guard; unique-ball guarantee can break — `_storyGrantTrackEndReward` (bug)
- [ISSUE-021] [P1] Two Master Ball sources collide — villain-track boss (Road 7, pre-HoF) + post-HoF broker = 2 per run — `_storyGrantTrackEndReward` (balance)
- [ISSUE-022] [P1] Road event-beats fire before in-city Gym Trainer / Gym Leader fights, not only on the route — `_tryFireRoadStoryBeats` (bug)
- [ISSUE-023] [P1] Player gimmick gate reads bare IIFE-private `sm` — always sees zero unlocked gimmicks — `_withStoryPlayerGimmickGate` (inconsistency)
- [ISSUE-024] [P1] aiDecision early-returns null on any choiceLock — AI cannot switch out of an immune/walled lock — `aiDecision` (bug)
- [ISSUE-025] [P1] Difficulty tiers scale only enemy stats; AI policy is byte-identical at every tier — no rising *challenge*, just a rising stat-wall — `applyFoeDifficultyScaling` (balance)
- [ISSUE-026] [P1] Measured curve violates the "regular<player, gym slightly-above, E4 EQUAL" intent — GL1-2 are 0.67-0.83× the player, gyms overshoot to 1.5×, E4 is ~1.70× (not equal) — `applyFoeDifficultyScaling` (balance)
- [ISSUE-027] [P1] Sleep duration roll uses bare `Math.random()` (RNG drift on every sleep proc) — `applyStatus` (bug)
- [ISSUE-028] [P1] League foe stat boost stacks multiplicatively despite comment claiming additive merge — `applyStoryLeagueFoeStatBoost` (bug)
- [ISSUE-029] [P1] Black Market shop from STORY_FEATURES_INTEGRATION.md §3 is still entirely unimplemented — `BLACK_MARKET_ITEMS` (inconsistency)
- [ISSUE-030] [P1] Three conflicting "canon" docs for the boss/endgame arc; code matches none cleanly — `BOSS_CONFIGS` (inconsistency)
- [ISSUE-031] [P1] `canMove` paralysis + confusion self-hit checks use bare `Math.random()` (RNG drift in story replays) — `canMove` (bug)
- [ISSUE-032] [P1] Sleep wake-check is off-by-one — ~1/3 of sleeps cost 0 turns (instant wake, mon acts same turn) — `canMove` (bug)
- [ISSUE-033] [P1] Sleep off-by-one: sleepDuration=1 wakes and attacks on its first turn (0 turns lost); effective sleep is 0-2 turns not 1-3 — `canMove` (bug)
- [ISSUE-034] [P1] `canMove` paralysis fizzle uses bare `Math.random()` while sibling freeze thaw uses `storyRngNext` — `canMove` (bug)
- [ISSUE-035] [P1] Casino Coin Flip outcome uses Math.random(), not seeded storyRngNext() — `casinoFlipSpin` (bug)
- [ISSUE-036] [P1] Casino Roulette winning cell chosen with Math.random(), not seeded RNG — `casinoRoulSpin` (bug)
- [ISSUE-037] [P1] Ball inventory + `catchMode` flag in STORY_FEATURES_INTEGRATION §1/§2/§5 do not match shipped code — `catchMode` (inconsistency)
- [ISSUE-038] [P1] The unique Master Ball can be wasted on any wild — soft-locks the Caged God capture (only 1% catch rate without it) — `catchThrow` (bug)
- [ISSUE-039] [P1] Unique Master Ball can be wasted on any non-boss wild (Crucible wild encounter), soft-locking the Caged God capture — VERIFIED still present post-merge — `catchThrow` (bug)
- [ISSUE-040] [P1] Toxic (badly-poison) counter `statusTurns` is not reset on switch-out — `clearVolatileOnSwitch` (bug)
- [ISSUE-041] [P1] Crucible League Run skips E1 and ends on the Rival — `_CRUCIBLE_LEAGUE_ROWS` are off-by-one row-ids — `crucibleLeagueRun` (bug)
- [ISSUE-042] [P1] proceedToNextBattle re-entry stacks duplicate cold-open overlays, wedging progression (the "After Badge One" stuck state) — `enterBattleEvent` (bug)
- [ISSUE-043] [P1] Post-HoF Crucible hub button gated on dead `bossArc.available` — never renders — `enterCity` (bug)
- [ISSUE-044] [P1] Retune risk: `data/builds/gen*.json` is a fallback mirror, not the live build source (`data/builds.csv` is authoritative) — `fetchSmogonSetsForGen` (data)
- [ISSUE-045] [P1] Choice-locked AI re-returns the locked move with zero immunity/wall check — spams 0-dmg moves forever — `getBestMove` (bug)
- [ISSUE-046] [P1] AI spams setup move into an active phazer — `score *= 0.25` penalty loses to near-zero attack scores — `getBestMove` (bug)
- [ISSUE-047] [P1] Illegal Dealer NPC encounter (STORY_FEATURES_INTEGRATION.md §3.5) still missing — `illegalDealer` (inconsistency)
- [ISSUE-048] [P1] City-8 legendary Mystery gate is bypassed if party has < 6 members — `isPreLeagueLegendaryMysteryGate` (bug)
- [ISSUE-049] [P1] Full Itinerary scaffolding (STORY_FEATURES_INTEGRATION.md §10) still entirely unimplemented — `itineraryProgress` (inconsistency)
- [ISSUE-050] [P1] `lastRemoteSeq` is updated *before* the remote handler runs — a thrown handler still bumps the watermark, so the failed update is permanently skipped — `lastRemoteSeq` (bug)
- [ISSUE-051] [P1] Save-migration integration test never exercises the migrate chain (vacuous pass) — `migrateStoryPreV15` (dx)
- [ISSUE-052] [P1] v14 trainer-assignment migration is bundled under `_loadedVer < 13`, so v13 saves skip it — `migrateStoryTrainerAssignmentsPreV14` (bug)
- [ISSUE-053] [P1] Secondary flinch/Stench write `defender.volatile.flinch` unguarded — throws if volatile missing (sibling _tryConfuse guards it) — `parseMoveEffects` (bug)
- [ISSUE-054] [P1] Many `parseMoveEffects` branches still use bare `Math.random()` — seeded story replays drift — `parseMoveEffects` (bug)
- [ISSUE-055] [P1] Data-driven boost block returns early, bypassing named-branch extra effects (Memento self-faint, Toxic Thread poison) — `parseMoveEffects` (bug)
- [ISSUE-056] [P1] Core damage roll, crit, and accuracy use bare `Math.random()` — the highest-impact drift sites — `parseMoveEffects-damage-core` (bug)
- [ISSUE-057] [P1] Damage formula divides un-truncated (fractional) A/D — Showdown floors atk/def stats first (±1 HP) — `parseMoveEffects-damage-formula` (bug)
- [ISSUE-058] [P1] Damage roll is continuous `0.85+rand*0.15` — never reaches 100%, so max-roll damage is unreachable — `parseMoveEffects-damage-roll` (bug)
- [ISSUE-059] [P1] Eight contact-triggered defender ability procs (Static / Poison Point / Flame Body / Cute Charm / Effect Spore / Toxic Chain / Cursed Body / Poison Touch) all use bare `Math.random()` — `parseMoveEffects-on-contact-abilities` (bug)
- [ISSUE-060] [P1] Contact-ability procs (Static / Poison Point / Flame Body / Cute Charm / Poison Touch / Toxic Chain / Cursed Body) all bare `Math.random()` — `parseMoveEffects-onhit-abilities` (bug)
- [ISSUE-061] [P1] PC_BOX_CAP is 30 in code but the canonical spec says 10 — `PC_BOX_CAP` (inconsistency)
- [ISSUE-062] [P1] Battle for Pokémon wager system (STORY_FEATURES_INTEGRATION.md §6) still missing — `pendingWager` (inconsistency)
- [ISSUE-063] [P1] Fire-type damaging moves do not thaw a frozen target (only flag-marked moves thaw) — `performAction` (bug)
- [ISSUE-064] [P1] HP-restore berry (Sitrus/Oran) eaten mid-hit suppresses Berserk / Wimp Out / Anger Shell HP-cross — `performAction` (bug)
- [ISSUE-065] [P1] Multi-hit contact moves skip all on-contact abilities/items (Rough Skin, Iron Barbs, Rocky Helmet, Static, etc.) — `performAction` (bug)
- [ISSUE-066] [P1] Multi-hit moves skip the Shield Dust / Sheer Force / Covert Cloak / Substitute secondary gate — `performAction` (bug)
- [ISSUE-067] [P1] Solar Beam bad-weather power halving is dead code — checks `"SolarBeam"` (no space) which never matches — `performAction` (bug)
- [ISSUE-068] [P1] Future Sight / Doom Desire resolve one turn too early (set to 2 turns; spec & Showdown require a 2-turn delay = 3) — `performAction` (bug)
- [ISSUE-069] [P1] Turn-order priority breakers (Quick Claw, speed-tie, rampage duration) use bare `Math.random()` — `playTurn` (bug)
- [ISSUE-070] [P1] End-of-turn residual block is not try-wrapped — any throw masks as "Turn skipped" + skips residuals — `playTurn` (bug)
- [ISSUE-071] [P1] `pushData` queue keeps advancing after a write fails — `_pushDataImpl` errors are swallowed to `console.warn`, leaving local state diverged from Supabase — `pushDataQueue` (bug)
- [ISSUE-072] [P1] `pvp_rooms` SELECT policy is `using (true)` — anyone with the public key can scrape every live match's full draft + battle state — `pvp_rooms_select` (security)
- [ISSUE-073] [P1] `remoteRowQueue` has no timeout — a hung `onOnlineRoomData` (e.g., a UI promise that never resolves) freezes ALL future remote updates — `remoteRowQueue` (bug)
- [ISSUE-074] [P1] Player-facing city actions read "Pokemon League" / "Pokemon Fan Club" (no diacritic) — `renderCityActions` (inconsistency)
- [ISSUE-075] [P1] Draft pick cards are click-only <div>s — keyboard/SR users cannot select a Pokémon — `renderDraft` (a11y)
- [ISSUE-076] [P1] `No Item` sentinel string used in 11 build slots is absent from `data/items.json` — `resolveCsvBuildEntry` (data)
- [ISSUE-077] [P1] Battle log (#battle-log) only cleared on returnToHome, not at battle start; previous fight's lines bleed in — `returnToHome` (bug)
- [ISSUE-078] [P1] showScreen() does no focus management on story-screen transitions — focus is orphaned — `showScreen` (a11y)
- [ISSUE-079] [P1] Mechanics-unlock docs still describe one-per-gym drip; code now unlocks all four at Colress/City 6 — `slotsUnlocked` (inconsistency)
- [ISSUE-080] [P1] Boss-mechanic hookup reads window.StoryMode.{BOSS_CONFIGS,bossMechanics*} but those live on test-only __storyTest — boss arc still dead in prod — `startBattle` (bug)
- [ISSUE-081] [P1] Boss/raid mechanics state never reset; bleeds into next ordinary Story fight — `startBattle` (bug)
- [ISSUE-082] [P1] Fresh run starts with 0 PokéBalls (spec says 5 at run start); 5 are gifted at first Mart instead — `startNewRun` (inconsistency)
- [ISSUE-083] [P1] Three mutually-incompatible story-narrative designs coexist; no doc is the single canon — `STORY_EVENTS_RAW` (inconsistency)
- [ISSUE-084] [P1] Reaper's Toll uses bare IIFE-private `storyRngNext` — guard always false, breaks seeded replays — `storyRngNext` (inconsistency)
- [ISSUE-085] [P1] Pokémon Trader (STORY_FEATURES_INTEGRATION.md §7) at City4 still missing — `traderOfferByCity` (inconsistency)
- [ISSUE-086] [P1] Speed-tie resolution + Quick Claw + lock-turn duration all use bare `Math.random()` in the turn loop — `turn-resolution` (bug)
- [ISSUE-087] [P1] `???` type used by gen1 `bide` and gen4 `curse` is not defined in `typeChart` — `typeChart` (data)
- [ISSUE-088] [P2] Turn-resolution catch masks any in-loop throw as "[Error: …. Turn skipped.]" — both moves abandoned, real bugs hidden (PT-001) — `__runLockedPvPTurnResolution` (bug)
- [ISSUE-089] [P2] ~250-line Caged God boss arc is dead code (unreachable) but still fully shipped — `_bossArcRenderSection` (refactor)
- [ISSUE-090] [P2] Caged God lead spec (§9 "visit Cities 2/5/8") contradicts shipped Crucible-hub collection; §14b omits the arc — `_bossArcRenderSection` (inconsistency)
- [ISSUE-091] [P2] Post-game lead "hunt" collapses to 3 buttons on one Crucible screen — no travel, no gating — `_bossArcRenderSection` (design)
- [ISSUE-092] [P2] Extra-track raid HP comment says miniRaid=(party-2)×, raid=(party-1)× base HP, but a separate ×1.3 _bossStatMult also multiplies maxHp — true HP is ~5.2×/6.5× at party 6 — `_bossHpScaleForKind` (inconsistency)
- [ISSUE-093] [P2] Two parallel story-flow engines coexist — new "unified" engine built but never wired (P2/P3 never done) — `_buildUnifiedStoryEvents` (refactor)
- [ISSUE-094] [P2] Dormant "unified flow engine" is now triple-orphaned — live dispatch is a 3rd design that leapfrogged it — `_buildUnifiedStoryEvents` (refactor)
- [ISSUE-095] [P2] Variants are rolled every run (not forced classic) — so variant Champion/post-HoF lines pointing at the dead broker/cage DO fire — `_CHAMPION_DIALOGUE_BY_VARIANT` (inconsistency)
- [ISSUE-096] [P2] Colress Signature-Z silently overwrites the last move; confirm warns only about item/gimmick — `_colressConfirm` (inconsistency)
- [ISSUE-097] [P2] Nature Rater cost badge shows "2000+" but TUTOR_COST_NATURE is a flat 2000 — `_costBadge` (inconsistency)
- [ISSUE-098] [P2] 7 duplicated `select('data').eq('id', roomId).single()` fetch+error blocks in online-pvp.js — `_hostRunResolution` (refactor)
- [ISSUE-099] [P2] Build "illegal-ability" pairs (672) are intended (hackmons/AAA + mega-form abilities); a naive legality check false-positives — `_isBuildAbilityIllegal` (inconsistency)
- [ISSUE-100] [P2] species.json Hisui formes are stale (gen8 snapshot) — Samurott-Hisui/Kleavor lack gen9 Sharpness, so every legal-tier build is dropped — `_isBuildAbilityIllegal` (data)
- [ISSUE-101] [P2] Cable Link gimmick gate contradicts CHANGELOG promise that Cable Link surfaces pre-unlock mechanics — `_makePlayerLinkBuild` (inconsistency)
- [ISSUE-102] [P2] Story autosave "💾 Saved" toast bypasses the polite live region — screen readers miss it — `_maybeShowSaveToast` (a11y)
- [ISSUE-103] [P2] "PC nearly full" warning threshold is `PC_BOX_CAP - 3` (27/30) but the spec calls for 80% (8/10) — `_pcRefresh` (inconsistency)
- [ISSUE-104] [P2] PC deposit/withdraw/release resets scroll to top via full innerHTML rewrite — `_pcRefresh` (dx)
- [ISSUE-105] [P2] Rivalry tab is mis-homed in the Pokémon Center — belongs in a progression/journal surface — `_pcRenderRivalJournalTab` (refactor)
- [ISSUE-106] [P2] Underground claims "Starters … aren't for sale" but starters are sellable (unsellable flag stripped) — `_pcRenderUndergroundTab` (inconsistency)
- [ISSUE-107] [P2] Villain-arc regular battles share 7 generic `tag:'villain'` grunts — never themed to the run's actual track (3 tracks have no matching grunt at all) — `_pickThemedTrainerForRole` (inconsistency)
- [ISSUE-108] [P2] _renderCrucible rebuilds a 17.7KB / 109-node innerHTML on every open + lead-collect + hard-mode toggle — `_renderCrucible` (perf)
- [ISSUE-109] [P2] Battle Frontier hub displays stale, weaker foe-scaling numbers than what applyStoryLeagueFoeStatBoost actually applies — `_renderFrontierHub` (inconsistency)
- [ISSUE-110] [P2] ~12 parallel event-presentation paths with 3 z-index layers and no single registry — `_renderNarrativeOverlay` (refactor)
- [ISSUE-111] [P2] Road beat clumping: 2 beats/road (villain road7 = 3, league = 7) play back-to-back, breaking pacing — `_resolveActiveRoadBeats` (balance)
- [ISSUE-112] [P2] Catch-tutorial gate comment claims "starting kit gives 5 balls" — fresh-run kit is actually 0 — `_shouldFireCatchTutorialBeforeBattle` (dx)
- [ISSUE-113] [P2] Story tutorial overlay (STORY_TUTORIAL_SCENES) is not a dialog — no role, focus trap, or ESC — `_showStoryTutorialScene` (a11y)
- [ISSUE-114] [P2] WANDER_AROUND_SPEC says "not yet implemented" but Wander Around shipped at SAVE_VER 23 — `_showWanderScreen` (inconsistency)
- [ISSUE-115] [P2] Current enemy curve is lumpy, not rising — GL4=GL5 dead zone (foe mult 1.0) and a GL8 quad-cliff (T3->T4 + IV 18->26 + gimmicks 2->3) — `_stageGatedFoeStatMult` (inconsistency)
- [ISSUE-116] [P2] Boss field-lock sets _bossWeatherLocked/_bossTerrainLocked but nothing reads them; weather decay path (20611/20617) ignores the lock — `_storyBossMechanicsBattleInit` (inconsistency)
- [ISSUE-117] [P2] Boss surge/immunity timers live on the active foe mon — lost on switch, stale on bench — `_storyBossMechanicsTurnTick` (bug)
- [ISSUE-118] [P2] Single `_bossPendingTelegraph` slot drops a phase when two mechanics telegraph on the same turn (mfBattle) — `_storyBossMechanicsTurnTick` (bug)
- [ISSUE-119] [P2] Shipped BOSS_CONFIGS uses surge/immunity/heal phases, not the EXPANSION_PLAN "multi-form transformation" — `_storyBossMechanicsTurnTick` (inconsistency)
- [ISSUE-120] [P2] In-catch "Up next" says "trainer/wild" but a battle-kind beat scene fires between catch and fight — `_storyComputeUpNext` (bug)
- [ISSUE-121] [P2] Foe party size matches `team.length` with per-role floors instead of the spec'd `min(6, 2+badges)` badge curve — `_storyEnemyPartySize` (balance)
- [ISSUE-122] [P2] Caged God removal (v24) is incomplete — residual content/help-text/achievements still reference the cut arc — `_storyEnsureMysteryIdentity` (inconsistency)
- [ISSUE-123] [P2] rollTrainerTeam's evo-stage cap uses cityIndexFromEventIndex on a ROW ID (not array index) — intro Rival gets cap 2 (fully evolved) instead of 0 (basics-only) — `_storyEvoStageCapForRow` (bug)
- [ISSUE-124] [P2] Master Ball granted by BOTH villain-boss victory and post-HoF Caged God, vs spec "1 per run" — `_storyGrantTrackEndReward` (inconsistency)
- [ISSUE-125] [P2] `_storyGrantTrackEndReward` has no internal idempotency guard — re-call double-grants Master Ball — `_storyGrantTrackEndReward` (bug)
- [ISSUE-126] [P2] Planned hatch animation + Fight Club transitions need reduced-motion + live-region design up front — `_storyHatchRevealScene` (a11y)
- [ISSUE-127] [P2] `_storyPickMysteryIdentity` uses bare `Math.random()` — Mystery Figure roll diverges across seeded replays — `_storyPickMysteryIdentity` (bug)
- [ISSUE-128] [P2] CSV-load cache invalidation pokes IIFE-private `_txMetaCache`/`_txGlobalMetaCached` — invalidation is dead — `_txMetaCache` (inconsistency)
- [ISSUE-129] [P2] `_variantMysteryOutro` is dead — `_MYSTERY_OUTRO_BY_VARIANT` keyed only by retired identities, never matches `the_first` — `_variantMysteryOutro` (bug)
- [ISSUE-130] [P2] All ~30 per-variant Mystery-Figure outros are dead — keyed by retired identities, never match `the_first` — `_variantMysteryOutro` (inconsistency)
- [ISSUE-131] [P2] FLOW §3 says wild grade is "keyed on sm.badges" via `_WILD_GRADE_CURVE_BY_BADGES`; shipped code keys it on city — `_wildGradeWeightsForCity` (inconsistency)
- [ISSUE-132] [P2] applyStatus dereferences `state.pActive.volatile.lockMove` / `state.fActive.volatile.lockMove` unconditionally (Uproar check) — throws if an active is null — `applyStatus` (bug)
- [ISSUE-133] [P2] Mystery Figure HP boost is 1.35 in code but 1.50 in both balance docs (canonical-curve drift) — `applyStoryLeagueFoeStatBoost` (inconsistency)
- [ISSUE-134] [P2] Dual-mega stone (Charizard/Mewtwo X vs Y) picked with bare Math.random — breaks seeded replay — `assignGimmickToBuild` (bug)
- [ISSUE-135] [P2] Team Yell villain track has no themed sprites — boss Piers falls back to generic Roughneck, mini-boss Marnie renders as Gladion — `BEAT_CANON_TRAINER` (data)
- [ISSUE-136] [P2] `battle`-kind beats (main.battle1/battle2) launch a generic trainer, not a themed one — no canon override — `BEAT_CANON_TRAINER` (inconsistency)
- [ISSUE-137] [P2] Memory growth is benign (linear, ~25 KB/turn, R² = 0.712 — noise-dominated) and **does not indicate a leak**; the mandate's "60 turn / quadratic = P1" threshold is not hit — `benchMemoryGrowth` (perf)
- [ISSUE-138] [P2] `perf-bench.mjs` calls `parseMoveEffects(move)` with the wrong arg-count; the reported 1.4 ms "median" is the cost of a thrown `TypeError`, not real per-move parsing — `benchParseMove` (perf)
- [ISSUE-139] [P2] Turn-loop max latency is **78–84 ms** with IQR 12 ms — within the harness target (50 ms median is OK) but max is 5× the median, indicating a per-turn outlier path — `benchTurn` (perf)
- [ISSUE-140] [P2] Caged God "Key" lead has zero cost — spec says it should demand strongest mon or steep gold — `bossCollectLead` (design)
- [ISSUE-141] [P2] buyArtifact has no interaction lock; double-submit safety rests solely on confirm-modal z-order — `buyArtifact` (bug)
- [ISSUE-142] [P2] Casino subtitle cream text on light-gold/rose panels fails WCAG AA contrast — `casino-game-subtitle` (contrast)
- [ISSUE-143] [P2] catch-system integration test asserts a stale "PC cap of 10" and passes on an incidental substring — same hollow-test pattern as safari-zone — `catch-system.test` (dx)
- [ISSUE-144] [P2] PC-cap integration test asserts cap 10 (stale) and passes only via false-positive regex match — `catch-system.test` (dx)
- [ISSUE-145] [P2] Unique Master Ball is spendable on any regular wild → Caged God capture becomes a 1%-per-throw grind — `catchThrow` (bug)
- [ISSUE-146] [P2] `sm.catchUnlocked` is written 3× but never read; spec §10 says it gates wild-route prompts — `catchUnlocked` (inconsistency)
- [ISSUE-147] [P2] `sm.settings.catchMode` toggle never implemented; catch shipped as always-on, 3 specs still gate on it — `catchUnlocked` (inconsistency)
- [ISSUE-148] [P2] autopilot-player classify() treats a cold-open as a city — pump fires city actions instead of dismissing the overlay, masking/causing the stuck-on-"After Badge One" report — `classify` (dx)
- [ISSUE-149] [P2] Comeuppance reflects 0 damage in all cases (twin Metal Burst works) — `Comeuppance` (bug)
- [ISSUE-150] [P2] Single Master Ball is a free consumable — spending it pre-cage leaves boss arc as a 1%-per-throw grind — `continuePostGame` (design)
- [ISSUE-151] [P2] Pre-boss-arc post-HoF saves may never receive the Master Ball / boss arc if parked at a city row on load — `continuePostGame` (bug)
- [ISSUE-152] [P2] Crucible "Mystery Figure" rematch uses out-of-bounds index 67 (array length is 67, max idx 66) — `crucibleMysteryFight` (bug)
- [ISSUE-153] [P2] Crucible "Rival Rematch" targets the Hall of Fame row (array idx 65), not the league rival — `crucibleRivalFight` (bug)
- [ISSUE-154] [P2] Crush Grip doesn't scale with target HP (constant ~2 dmg); siblings do — `Crush Grip` (bug)
- [ISSUE-155] [P2] `deepClone` falls back to `JSON.parse(JSON.stringify(...))` — silently drops Set/Map/Date/undefined/circular refs across the entire snapshot pipeline — `deepClone` (refactor)
- [ISSUE-156] [P2] Design checklist's load-bearing "CSS block = battle.html lines 16-4156" guardrail is wrong by ~3700 lines — `DESIGN_CONSISTENCY_CHECKLIST.md` (dx)
- [ISSUE-157] [P2] Malva (Kalos E1) has a victory line but no intro pool in `TRAINER_QUOTES_BY_NAME` — `ELITE_VICTORY_LINES` (inconsistency)
- [ISSUE-158] [P2] Leech Seed end-of-turn drain ignores Magic Guard (holder loses HP, seeder heals) — `endOfTurnEffects` (bug)
- [ISSUE-159] [P2] Partial-trap (Bind / Fire Spin / Whirlpool / Sand Tomb) end-of-turn damage ignores Magic Guard — `endOfTurnEffects` (bug)
- [ISSUE-160] [P2] Relic vs Artifact used interchangeably for one object across label/key/fn/state — `enterArtifactShop` (inconsistency)
- [ISSUE-161] [P2] PLAN COLLISION — Daycare unlock is keyed on the "Gym Leader 1" event name, not a city; redesign wants C2/C4/C6 — `enterDaycare` (refactor)
- [ISSUE-162] [P2] Poké Center never clears Fatigue, yet the in-game bulletin tells players a Center stay clears it — `enterPokemonCenter` (bug)
- [ISSUE-163] [P2] Validator reports 6925 "CSV alternative" occurrences but every build file uses arrays — false positive — `expandCommaAlternatives` (dx)
- [ISSUE-164] [P2] Facility debut cities disagree 3 ways (code FACILITY_DEBUT_CITY vs both balance docs vs in-code comments) — `FACILITY_DEBUT_CITY` (inconsistency)
- [ISSUE-165] [P2] Early-game softening is a per-CITY table in code, not the per-event named constants both balance docs describe — `FOE_STAT_NERF_BY_CITY` (inconsistency)
- [ISSUE-166] [P2] No sleep clause in story + AI scores Spore at 100 → up to 3-turn lock loops, amplified by high-tier stat bloat (fairness risk) — `getBestMove` (balance)
- [ISSUE-167] [P2] When every damaging move is immune (score 0), AI throws a 0-dmg attack instead of switching/using status — `getBestMove` (inconsistency)
- [ISSUE-168] [P2] Salac Berry grants a phantom 1.5x Speed while merely held at <=25% HP (not consumed) — `getEffectiveSpeed` (bug)
- [ISSUE-169] [P2] City-3 HUD/route name falls back to "City 3" — GYM_CITY_LEADER_EVENT array-index keys trainerAssignments (row-id keyed) — `getStoryDisplayTownNameForCityRow` (bug)
- [ISSUE-170] [P2] 36 references to 18 distinct `global.__*` variables across `online-pvp.js` — the PvP module mutates host-side state through ad-hoc globals instead of a single observable — `global_state_coupling` (refactor)
- [ISSUE-171] [P2] City-3 display name always falls back to "City 3" — GYM_CITY_LEADER_EVENT returns an array index, but trainerAssignments is keyed by row ID — `GYM_CITY_LEADER_EVENT` (bug)
- [ISSUE-172] [P2] In-game Help "Catching" section still points players to the cut Caged God arc — `helpText` (inconsistency)
- [ISSUE-173] [P2] City-8 "Required" legendary handoff silently downgrades to a normal Professor gift when the party is below cap — `isPreLeagueLegendaryMysteryGate` (inconsistency)
- [ISSUE-174] [P2] `_loadedVer < 14` block missing — v14 trainer-name migration never runs for saves stamped at exactly v13 — `load` (bug)
- [ISSUE-175] [P2] Migration chain is sound but unobservable — no boot-time shadow validation — `load` (dx)
- [ISSUE-176] [P2] Pre-merge saves with partial unlockedGimmicks are not re-derived on load — Tera/Z silently withheld until next milestone win — `load` (bug)
- [ISSUE-177] [P2] No save migration coerces stale `sm.mysteryIdentity`; pre-v22 saves render degraded MF reveal until the fight — `load` (bug)
- [ISSUE-178] [P2] 6 builds in the gen*.json mirror omit `nature`; the CSV source has natures for all 17,398 rows (mirror drift) — `loadBuildsCSV` (inconsistency)
- [ISSUE-179] [P2] Engine-only `loadGameData` parse is ~308 ms (isolated from JSDOM), >1.5× the 200 ms boot target and scales with every new data table — `loadGameData` (perf)
- [ISSUE-180] [P2] `logMsg` runs an O(903-keys) `Object.keys(tooltipDict)` scan on every log line — 0.32 ms median, 13.9 ms max per call — `logMsg` (perf)
- [ISSUE-181] [P2] 14 species have their ENTIRE standard-tier build pool tagged illegal — designed sets are dropped, foe falls back to randbats/Tackle — `makeBuild` (data)
- [ISSUE-182] [P2] Pre-v15 post-HoF saves are forced back through the Mystery Figure climax — postHofMysteryClimaxDone migration shadowed by default boolean — `migrateStoryPreV15` (bug)
- [ISSUE-183] [P2] 10 of 12 `.modal` overlays lack `role="dialog"` / `aria-modal` / labelledby — `modal-dialog-roles` (a11y)
- [ISSUE-184] [P2] Only `modal-summary` handles Escape — settings, abandon, alert, confirm modals trap keyboard users — `modal-escape-key` (a11y)
- [ISSUE-185] [P2] Modals restore focus on close but never move focus INTO the dialog on open — `openModal` (a11y)
- [ISSUE-186] [P2] Precondition moves unimplemented — deal damage that should fail (Dream Eater, Thunderclap, Synchronoise) — `parseMoveEffects` (bug)
- [ISSUE-187] [P2] Burn halving & Ice Scales key off `move.cat`, not `_effectiveCat` — wrong for Photon Geyser / Shell Side Arm — `parseMoveEffects-effectiveCat-burn` (bug)
- [ISSUE-188] [P2] All damage modifiers collapsed into one multiply + single floor — Showdown floors per modifier (multi-HP drift) — `parseMoveEffects-modifier-pipeline` (inconsistency)
- [ISSUE-189] [P2] In-game help text still says "PC Storage (cap 10)" — actual PC_BOX_CAP is 30 — `PC_BOX_CAP` (inconsistency)
- [ISSUE-190] [P2] Pokémon Center storage rows are mouse-only clickable divs (no keyboard access) — `pcRenderStorage` (a11y)
- [ISSUE-191] [P2] 2-5 multi-hit distribution is 33/33/17/17, not the modern 35/35/15/15 — `performAction` (bug)
- [ISSUE-192] [P2] Multi-hit moves reuse one damage roll & one crit check for every hit (no per-hit independence) — `performAction` (bug)
- [ISSUE-193] [P2] OHKO moves use the generic accuracy gate — affected by evasion/accuracy stages, Compound Eyes, Gravity; no higher-level auto-fail — `performAction` (bug)
- [ISSUE-194] [P2] "Vitamin" names three distinct systems — IV items, casino prize, EV voucher — `PERM_BOOST_ITEMS` (inconsistency)
- [ISSUE-195] [P2] Nurse Joy first-Center tutorial says PC has "ten slots" but PC_BOX_CAP is 30 — `playStoryTutorial` (inconsistency)
- [ISSUE-196] [P2] End-of-turn residuals always resolve player-active-first, not in Speed order — `playTurn` (bug)
- [ISSUE-197] [P2] Turn-loop median **23–38 ms / p95 35–53 ms / max 46–58 ms** in jsdom — production with `settings.animations=true` adds bounded `sleep()` delays on top, but the jsdom number IS the production floor when animations are off — `playTurn` (perf)
- [ISSUE-198] [P2] 29 of 31 mart/dept catalog items (potion, superPotion, X items, orbs, etc.) have no entry in `data/items.json` — `POKEMART_ITEMS` (data)
- [ISSUE-199] [P2] Mart/Dept consumables (30 ids: potion, xAttack, sunOrb, evResetCharm…) are a self-contained namespace, NOT entries in items.json — `POKEMART_ITEMS` (data)
- [ISSUE-200] [P2] `_storyBattleEntryBusy` can latch true on a cold-open / beat-scene continuation throw → soft-locks "Enter Gym / Continue Route" — `proceedToNextBattle` (bug)
- [ISSUE-201] [P2] City-0 starter pick is drawn from a pure-G4 (weakest tier) pool — `PROF_ROLLS` (balance)
- [ISSUE-202] [P2] Mystery swap picker mislabels BST grade as "Power tier (1-4)" — `profAccept` (inconsistency)
- [ISSUE-203] [P2] README calls catch / PC / Underground / Safari / boss-arc "upcoming"; all are shipped — `README` (inconsistency)
- [ISSUE-204] [P2] Catch screen result/throw text has no aria-live; outcomes silent to screen readers — `renderCatchScreen` (a11y)
- [ISSUE-205] [P2] City-hub Mystery Figure NPC sprite is hard-coded to Cyrus, ignoring `sm.mysteryIdentity` rotation — `renderCityActions` (bug)
- [ISSUE-206] [P2] Pokémon Center copy promises a "Heal" that no longer exists (full-heal is automatic) — `renderCityActions` (bug)
- [ISSUE-207] [P2] ISSUE-038 is marked fixed but `No Item` is still absent from items.json and 11 build slots still reference it — `resolveCsvBuildEntry` (inconsistency)
- [ISSUE-208] [P2] `_trainerPoolCache` is an unbounded Map (keyed on type+gens) with no eviction — Fight Club draft / story-pool variety will grow it without limit — `rollTrainerTeam` (perf)
- [ISSUE-209] [P2] Safari unlock spec'd "after badge 3 OR City3" but code (and REDESIGN) fix it firmly at City4 — `SAFARI_ENTRY_COST` (inconsistency)
- [ISSUE-210] [P2] safari-zone integration test gives false confidence — asserts stale hard-coded weights and matches "1.25" anywhere in the spec doc — `safari-zone.test` (dx)
- [ISSUE-211] [P2] STORY_MODE_FLOW pins SAVE_VER at 15/17; shipped SAVE_VER is 21 — `SAVE_VER` (inconsistency)
- [ISSUE-212] [P2] SAVE_VER stays at 19 despite an implicit v20 schema cleanup that runs on every load — `SAVE_VER` (dx)
- [ISSUE-213] [P2] SAVE_VER=23 but migration chain stops at PreV22 — no migrateStoryPreV23 step (v23 added wanderByEventIdx, back-filled unconditionally) — `SAVE_VER` (dx)
- [ISSUE-214] [P2] Pending Healing Wish / Lunar Dance flags bleed into next battle and auto-heal its lead — `selectPartyMember` (bug)
- [ISSUE-215] [P2] sellItem trusts caller-supplied sellPrice instead of re-deriving from the catalog — `sellItem` (bug)
- [ISSUE-216] [P2] 6 silent `catch (e) {}` blocks in online-pvp.js swallow all errors without logging — `setBattleLogHtml` (dx)
- [ISSUE-217] [P2] Display names accepted up to 24 chars with zero sanitization — flow into innerText today, but one careless innerHTML downstream becomes XSS — `setDisplayName` (security)
- [ISSUE-218] [P2] settings.animations defaults to true and is never seeded from prefers-reduced-motion — `settings-animations-init` (a11y)
- [ISSUE-219] [P2] Snow Warning maps to Hail (gen-8) instead of gen-9 Snow — `setWeatherFromAbility` (engine-fidelity)
- [ISSUE-220] [P2] Anomaly seeds fire via low-z `showGameAlert` on the same tick as the row's flow — can paint behind/over other overlays — `showGameAlert` (inconsistency)
- [ISSUE-221] [P2] showGameConfirm overwrites a pending _gameConfirmResolve, orphaning the first awaiter — `showGameConfirm` (bug)
- [ISSUE-222] [P2] anime.js move-FX engine ignores prefers-reduced-motion — heaviest motion bypasses the CSS catch-all — `showMoveEffect` (a11y)
- [ISSUE-223] [P2] Victory overlay is a plain `<div>` — no dialog role, no focus on Continue, ESC ignored — `showVictoryOverlay` (a11y)
- [ISSUE-224] [P2] Fresh run starts with 0 Poké Balls; skipping the optional City-0 Mart silently no-ops the catch tutorial — `startNewRun` (bug)
- [ISSUE-225] [P2] Achievements `caged_god` / `r_caged_god` are permanently unobtainable (dead arc) — `STORY_ACHIEVEMENTS` (inconsistency)
- [ISSUE-226] [P2] Caged God achievements (caged_god, r_caged_god) are permanently unearnable after v24 arc cut — `STORY_ACHIEVEMENTS` (inconsistency)
- [ISSUE-227] [P2] STORY_MODE_FLOW says timeline is "68 rows"; STORY_EVENTS_RAW actually has 67 rows — `STORY_EVENTS_RAW` (inconsistency)
- [ISSUE-228] [P2] Story spine is a hardcoded linear array; narrative layer is data-driven but no structural side-story/random-pool slots — `STORY_EVENTS_RAW` (refactor)
- [ISSUE-229] [P2] Recurring facility-flavor pool covers 8 services; Safari / Stone Sage / Stone Shop / Dept Store have none — `STORY_FACILITY_QUOTES` (inconsistency)
- [ISSUE-230] [P2] De-scoped features (Black Market, Illegal Dealer, Trader, Wager, full Itinerary) still presented as active spec — precise doc-edit list — `STORY_FEATURES_INTEGRATION` (dx)
- [ISSUE-231] [P2] Surviving canonical specs + code link to docs deleted in the cleanup (dangling references) — `STORY_MODE_FLOW` (inconsistency)
- [ISSUE-232] [P2] Story tone variants recolor nameplate text but not its yellow background — fails WCAG AA contrast — `story-dialog-nameplate` (a11y)
- [ISSUE-233] [P2] Story dialogue/NPC quote text is not in a live region — narration is silent to screen readers — `story-dialog-text` (a11y)
- [ISSUE-234] [P2] Verbatim gold-HUD markup re-inlined 11× — the "re-inlined block 25 times" anti-pattern CLAUDE.md warns about — `story-gold-icon` (refactor)
- [ISSUE-235] [P2] Help screen still advertises the cut Caged God / Subject Zero / Master-Ball quest — `storyHelpText` (inconsistency)
- [ISSUE-236] [P2] STORY_NARRATIVE_VARIANTS.md presents a cut 8-variant design as "canonical" (future-prompt-rebuild trap) — `STORYLINE_VARIANTS` (inconsistency)
- [ISSUE-237] [P2] Cosmetic-skin roll references bare `sm` AND bare `storyRngNext` — double scope leak, never seeded — `storyRngNext` (inconsistency)
- [ISSUE-238] [P2] 351 it.todo() stubs across 3 move-category test files — cluster enumeration — `tests/moves/by-category` (test-gap)
- [ISSUE-239] [P2] 56 Gym Leaders have no entry in `TRAINER_QUOTES_BY_NAME` — fall through to generic 6-line `Gym Leader` pool — `TRAINER_QUOTES_BY_NAME` (inconsistency)
- [ISSUE-240] [P2] Trick / Switcheroo swap is one-directional — the user's item is destroyed — `Trick` (bug)
- [ISSUE-241] [P2] Tooltip-only data (type chart, move info, status terms) is `onmousemove`-gated — keyboard & touch get nothing — `type-badge` (a11y)
- [ISSUE-242] [P2] Long story replay shows steady, non-plateauing heap growth (~22–67 KB/turn, R²=0.99 over 250 battles) driven by per-turn / per-distinct-foe DOM scaffolding that GC + reset() do not reclaim — `updateBattleUI` (perf)
- [ISSUE-243] [P2] Long story replay shows linear (non-quadratic) heap + DOM-node growth that does not plateau — ~0.275 MB and ~52 sprite-container nodes retained per battle — `updateBattleUI` (perf)
- [ISSUE-244] [P2] STORY_3TRACK_IMPL_PLAN reads as a forward plan but is mostly SHIPPED — only PR-1 marked done — `VILLAIN_STORY_BEATS` (inconsistency)
- [ISSUE-245] [P2] EVOLUTION_FLOW_REBUILD.md header says "Status: Plan — review before implementation" but the system fully shipped — `VOUCHER_KEYS` (inconsistency)
- [ISSUE-246] [P3] Modals have aria-modal + Escape but no Tab focus trap — keyboard focus can leave the dialog — `__pbsGlobalEscBound` (a11y)
- [ISSUE-247] [P3] Heal phase (+25% maxHp) can push a raid boss back ABOVE the HP threshold the player just crossed — `_applyBossPhaseEffect` (inconsistency)
- [ISSUE-248] [P3] Dual story-vs-Frontier path in `_applyStoryBuildPowerTier` is mutually exclusive and safe — legacy branch is NOT removable (in-scope MF needs it) — `_applyStoryBuildPowerTier` (inconsistency)
- [ISSUE-249] [P3] Lead→city mapping duplicated (`_BOSS_LEAD_CITIES` const vs inline `_leadCity` literal) — `_BOSS_LEAD_CITIES` (inconsistency)
- [ISSUE-250] [P3] Caged God uses three names for one entity (Specimen 0001 / Subject Zero / Subject 0001) without a stated rule — `_bossArcCheckCageUnlock` (inconsistency)
- [ISSUE-251] [P3] _bossArcRenderSection rebuilt in full inside every _renderCrucible re-render (adds ~6ms of the 30ms) — `_bossArcRenderSection` (perf)
- [ISSUE-252] [P3] Non-hub Caged God render path is effectively dead post-HoF (player can never be at City 2/5/8) — `_bossArcRenderSection` (design)
- [ISSUE-253] [P3] Entire Caged God boss-arc subsystem is dead code after v24 removal — `_bossArcRenderSection` (refactor)
- [ISSUE-254] [P3] Solo-raid HP is 6.5× base, not the documented (maxParty-1)=5× — stat-mult and HP-scale compound on HP — `_bossHpScaleForKind` (inconsistency)
- [ISSUE-255] [P3] CONFIRMED CLEAN — PC overflow at party-cap + 30/30 shows explicit message; sell/release path exists — `_catchHandleSuccess` (bug)
- [ISSUE-256] [P3] Eggs occupy a party slot against the catch/withdraw cap but foe size matches only non-egg fighters — eggs silently shrink your catchable roster AND your opponent — `_catchHandleSuccess` (inconsistency)
- [ISSUE-257] [P3] Regular wild encounter with zero balls shows greyed buttons but no "out of balls" message — `_catchRender` (dx)
- [ISSUE-258] [P3] Variant Champion / rival dialogue narratively routes player to the dead broker + cage — `_CHAMPION_DIALOGUE_BY_VARIANT` (inconsistency)
- [ISSUE-259] [P3] "Free" badge wording inconsistent — "1st Free" vs "Free" vs "Claimed" vs "Locked" — `_costBadge` (inconsistency)
- [ISSUE-260] [P3] 9 Gen-2-legacy "isBerry" items are dead data — no engine handler and never referenced by any build — `_onBerryEaten` (data)
- [ISSUE-261] [P3] Dead CSS selector #story-pc-tab-journal-btn — no such tab button exists in the Poké Center — `_pcRefresh` (dx)
- [ISSUE-262] [P3] `_pendingProfRoll` (singular) only ever assigned null — dead variable shadowing live `_pendingProfRolls` — `_pendingProfRoll` (refactor)
- [ISSUE-263] [P3] Inert `_permBoostsRead`/`_permBoostTotal` stubs (+ window export) have zero callers — fully dead — `_permBoostsRead` (refactor)
- [ISSUE-264] [P3] `_pickCityQuoteLine` deliberately uses bare `Math.random` — drift across save reloads — `_pickCityQuoteLine` (inconsistency)
- [ISSUE-265] [P3] Variant roll + Mystery identity use bare `Math.random()` at run construction — non-deterministic across seeded replays — `_pickRandomStorylineVariant` (bug)
- [ISSUE-266] [P3] League-road narrative "clumping" — 6 story beats fire back-to-back before the Champion (the audit §4 flow bug, still unfixed in the live path) — `_playStoryBeatQueue` (inconsistency)
- [ISSUE-267] [P3] Sprite preloading is unbounded — each `getSprite()` call adds a `new Image()` to a global cache with no eviction; a long story run can preload 1000+ images — `_preloadedImages` (perf)
- [ISSUE-268] [P3] Casino debits floor gold at Math.max(0, gold-bet), which would silently mask a future bet-validation regression — `_refreshCasinoGoldPill` (dx)
- [ISSUE-269] [P3] Crucible "Pokémon Center" facility re-renders the Caged God section a second time (below the Underground sell list) — `_renderCrucible` (design)
- [ISSUE-270] [P3] Grade badge prefix differs between prof pick cards (G#) and swap slots (T#) — `_renderProfChoices` (inconsistency)
- [ISSUE-271] [P3] "Reveal lands inside first ~10 minutes" comment is wrong — first villain beat is post-Gym-2 (road2) — `_ROAD_BY_ARRAY_IDX` (inconsistency)
- [ISSUE-272] [P3] Safari curve key [3] ("first unlock") is dead code — Safari actually unlocks at 4 badges, so first visit uses the harsher [4] curve — `_SAFARI_GRADE_CURVE_BY_BADGES` (inconsistency)
- [ISSUE-273] [P3] Safari grade weights are a per-badge curve in code, but STORY_MODE_FLOW.md §4 still specs the old flat g1:3/g2:22/g3:50/g4:25 — `_SAFARI_GRADE_CURVE_BY_BADGES` (inconsistency)
- [ISSUE-274] [P3] Safari grade weights are now badge-keyed — spec/CODEBASE_MAP still cite the old static g1:3/g2:22/g3:50/g4:25 — `_safariGradeWeightsForBadges` (inconsistency)
- [ISSUE-275] [P3] Safari grade weights diverge from the canonical spec (`g1:3/g2:22/g3:50/g4:25`) — code is a badge-staged curve; spec is stale — `_safariGradeWeightsForBadges` (data)
- [ISSUE-276] [P3] CONFIRMED CLEAN — catch tutorial fires exactly once; mid-tutorial reload cannot refire or lock — `_shouldFireCatchTutorialBeforeBattle` (bug)
- [ISSUE-277] [P3] Post-HoF orientation tip frames the Mystery Figure as un-fought, but the row-67 climax already unmasked it — `_showOrientationTipThenCity` (inconsistency)
- [ISSUE-278] [P3] CORRECTION to prior audit: storyline variant is rolled randomly every run, NOT forced to 'classic' — `_storyActiveVariant` (dx)
- [ISSUE-279] [P3] faintPhase counts the active foe as "fainted" mid-tick if it is at 0 HP before the swap — `_storyBossMechanicsTurnTick` (bug)
- [ISSUE-280] [P3] bossMechanicsTurnTick per-turn cost is ~1.5us (foeParty.filter is NOT wasteful); only _showBossBanner DOM is non-trivial and fires ~5x/battle — `_storyBossMechanicsTurnTick` (perf)
- [ISSUE-281] [P3] Redundant tier branches in `_storyBuildTierForEvent` (dead duplicate conditions) — `_storyBuildTierForEvent` (refactor)
- [ISSUE-282] [P3] `_storyBuildTierForEvent` Basic-Trainer branch has a dead `b>=5` arm — route fodder jumps T2→T4 at post-game with no T3 step (half-applied curve edit) — `_storyBuildTierForEvent` (bug)
- [ISSUE-283] [P3] "wild < gym staff" ladder is violated — Basic Trainer and Gym Trainer 1 share the SAME tier at every badge count — `_storyBuildTierForEvent` (inconsistency)
- [ISSUE-284] [P3] Basic Trainer build-tier ladder collapses at Stage 2 — same tier as Gym Trainers despite the "one tier below" comment — `_storyBuildTierForEvent` (balance)
- [ISSUE-285] [P3] `_storyEnemyMechKeys` doesn't guard against missing `sm.settings` — could throw on a malformed save — `_storyEnemyMechKeys` (dx)
- [ISSUE-286] [P3] Mystery Figure sprite is now `Red` (the_first); the `'Cyrus'` fallback at enterBattleEvent is dead — `_storyEnsureMysteryIdentity` (inconsistency)
- [ISSUE-287] [P3] Stale comment on `_storyGrantTrackEndReward` — claims scene-queue piggy-back that is structurally impossible — `_storyGrantTrackEndReward` (dx)
- [ISSUE-288] [P3] Extra-track raid EXP-Share reward + boss BOSS_MECHANICS are partly data-only — engine wiring deferred (mechanics are no-ops that only record) — `_storyGrantTrackEndReward` (dx)
- [ISSUE-289] [P3] CONFIRMED CLEAN — party-cap curve = min(6, 2+badges) with no off-by-one; foe sizing matches — `_storyMaxPartySize` (bug)
- [ISSUE-290] [P3] Legacy storyline picker is dead UI — hidden DOM + uncalled renderer + unreachable card handlers, superseded by sm.tracks — `_tcRenderStorylineGrid` (refactor)
- [ISSUE-291] [P3] `_validateTrainerData` logs a success `console.log` on every boot (ungated) — `_validateTrainerData` (dx)
- [ISSUE-292] [P3] Variant rival quote pools are uneven — several phases have a single line; many phases absent — `_VARIANT_RIVAL_QUOTES` (refactor)
- [ISSUE-293] [P3] Wild grade keyed on CITY (STORY_WILD_GRADE_BY_CITY), not badges — spec §3/§15f say _WILD_GRADE_CURVE_BY_BADGES (which does not exist) — `_wildGradeWeightsForCity` (inconsistency)
- [ISSUE-294] [P3] CONFIRMED CLEAN — mechanics unlock gate has no leak on any player or enemy path — `_withStoryPlayerGimmickGate` (bug)
- [ISSUE-295] [P3] items.json defines 93 mega stones but the engine recognizes only 51 — 45 non-canonical stones are inert data — `ALL_MEGA_STONES` (data)
- [ISSUE-296] [P3] Anomaly seeds are keyed by row ID but several land on mismatched event types vs their prose — `ANOMALY_SEEDS` (data)
- [ISSUE-297] [P3] Latent state-bleed: artifact battle-flags reset is behind an empty-artifacts early-return (same init-inside-guard shape as the fixed boss-bleed) — `applyArtifactBattleEffects` (bug)
- [ISSUE-298] [P3] `applyFoeDifficultyScaling`'s "additive league boost" comment narrates a fix that was never implemented — `applyFoeDifficultyScaling` (dx)
- [ISSUE-299] [P3] Spec §8 says league boost stacks multiplicatively with difficulty; code now stacks additively (the cliff was fixed) — `applyFoeDifficultyScaling` (inconsistency)
- [ISSUE-300] [P3] Spec §8 says league boost stacks MULTIPLICATIVELY with difficulty; code now stacks ADDITIVELY — `applyStoryLeagueFoeStatBoost` (inconsistency)
- [ISSUE-301] [P3] Magma/Aqua bosses flash the same telegraph banner twice in the first two turns — `BOSS_CONFIGS` (inconsistency)
- [ISSUE-302] [P3] `BOSS_MECHANICS` stub object is dead (never called); its "engine wiring is a no-op" comment is now false — `BOSS_MECHANICS` (dx)
- [ISSUE-303] [P3] `BOSS_MECHANICS` registry (`~42172`) is dead — pushes to `battle._mechanics`, which is never read — `BOSS_MECHANICS` (refactor)
- [ISSUE-304] [P3] Dead `build.tired` fatigue field still written/backfilled at 5 sites, read in zero gameplay paths — `build.tired` (refactor)
- [ISSUE-305] [P3] Extra-raid stat scaling compounds `_storyStatMult` × `_bossStatMult` × `_bossHpScale`; the doc comment omits `_storyStatMult` — `buildPokemon` (inconsistency)
- [ISSUE-306] [P3] buyItem cheap-consumable path is unguarded but synchronous; only the confirm-gated branch can interleave — `buyItem` (inconsistency)
- [ISSUE-307] [P3] Achievements caged_god / r_caged_god are permanently unobtainable — `caged_god` (data)
- [ISSUE-308] [P3] Roulette doc comment promises a color-row payout the code never pays — `casinoRoulSpin` (inconsistency)
- [ISSUE-309] [P3] Integration test asserts stale "PC cap of 10" and never checks the real PC_BOX_CAP=30 — `catch-system.test.js` (dx)
- [ISSUE-310] [P3] `sm.catchUnlocked` is written by migration + newStoryRun but never read anywhere — `catchUnlocked` (dx)
- [ISSUE-311] [P3] sm.catchUnlocked written by defaults + v15 migration + newStoryRun but read nowhere (live gate is sm.catchTutorialDone) — `catchUnlocked` (dx)
- [ISSUE-312] [P3] CHAMPION_VICTORY_LINES['Hau'] is dead — Hau is an Elite Trainer, never a Champion — `CHAMPION_VICTORY_LINES` (inconsistency)
- [ISSUE-313] [P3] CHANGELOG 2026-05-21 entry claims Cable Link is ungated but the implementation in v18 explicitly gates it — `CHANGELOG` (inconsistency)
- [ISSUE-314] [P3] `createRoom` hardcodes Postgres SQLSTATE `23505` for unique-violation detection — defensive `.includes('duplicate')` fallback exists but the magic number is undocumented — `createRoom_23505` (refactor)
- [ISSUE-315] [P3] Crucible rematch pickers use bare Math.random — breaks the seeded-replay contract for post-game — `crucibleGymPick` (design)
- [ISSUE-316] [P3] ELITE_VICTORY_LINES['Molayne'] is dead — Molayne is an Elite Trainer, not an E1–E4 boss — `ELITE_VICTORY_LINES` (inconsistency)
- [ISSUE-317] [P3] Leech Seed drain is processed AFTER burn/poison/toxic damage (canon order is before) — `endOfTurnEffects` (bug)
- [ISSUE-318] [P3] Relic Annex intro uses plain-text `_storyShowOneTimeTip`; every other facility uses a sprite-backed scene — `enterArtifactShop` (inconsistency)
- [ISSUE-319] [P3] enterArtifactShop and enterShop lack the _storyTryBeginInteraction guard used by other facility entries — `enterArtifactShop` (dx)
- [ISSUE-320] [P3] City0 welcome tip says the Underground "buys … never your starter" but starters are sellable — `enterCity` (inconsistency)
- [ISSUE-321] [P3] Crucible sub-sections improve wayfinding but the orientation tip + "Mystery vs Caged God" disambiguation still lean on long alert text — `enterCrucible` (design)
- [ISSUE-322] [P3] Poké Center chip sits in "Heal & Team" section with a "Free" badge but performs no heal interaction — `enterPokemonCenter` (inconsistency)
- [ISSUE-323] [P3] Story facility regions use weak lowercase aria-labels ("story pokemoncenter", "story link") — `enterPokemonCenter` (dx)
- [ISSUE-324] [P3] `enterProfessor` duplicates gimmick-gate logic inline instead of using `_withStoryPlayerGimmickGate` — `enterProfessor` (dx)
- [ISSUE-325] [P3] `enterProfessor` reuses `_pendingProfChoices` across city visits at the same cityIdx — stale picks may persist past spec'd one-shot pool — `enterProfessor` (bug)
- [ISSUE-326] [P3] Professor flavor quote uses bare Math.random(), breaking seeded replay determinism — `enterProfessor` (bug)
- [ISSUE-327] [P3] Empty-choices Professor path shows status but renders no body buttons — `enterProfessor` (bug)
- [ISSUE-328] [P3] Exp Share Voucher item (3TRACK_IMPL_PLAN PR-5) never shipped; `sm.inventory.expShareVoucher` is dead init — `expShareVoucher` (inconsistency)
- [ISSUE-329] [P3] `expShareVoucher:0` inventory field is dead — no reader, no writer; extra-raid reward grants vitamins instead — `expShareVoucher` (data)
- [ISSUE-330] [P3] Service-availability timeline reference (Task 1 deliverable) — first-appearance / reappear / unlock map — `FACILITY_DEBUT_CITY` (data)
- [ISSUE-331] [P3] Crucible-reachable Frontier surrender uses raw window.confirm — drops fullscreen, breaks modal convention — `frontierSurrender` (dx)
- [ISSUE-332] [P3] Gauntlet score readout is a plain div with no live region — score changes are silent to SR — `gauntlet-score` (a11y)
- [ISSUE-333] [P3] Paralysis tooltip says "Speed quartered" but engine halves speed (0.5) — stale Gen 1-6 text vs Gen 7+ behavior — `getDownStatusLabel` (inconsistency)
- [ISSUE-334] [P3] Rival phase enum skips 1 (EARLY rival returns phase 2), leaving a dead phase-1 dialogue pool — `getRivalEncounterPhase` (inconsistency)
- [ISSUE-335] [P3] Sprite preload cache `_preloadedImages` is still an unbounded Object with no eviction — every distinct (name, shiny, back) pins an Image() for the session — `getSprite` (perf)
- [ISSUE-336] [P3] Two disjoint "beat" systems — row-id `STORY_BEATS` (cold-opens) vs sceneKey `*_STORY_BEATS` (3-track) — `getStoryBeatForRow` (refactor)
- [ISSUE-337] [P3] Featured Mega/Ultra stones (bought one-per-city at 5x/3x) are sellable from the bag at half list price — `getStoryFeaturedItems` (dx)
- [ISSUE-338] [P3] Grass Whistle never puts the target to sleep — `Grass Whistle` (bug)
- [ISSUE-339] [P3] CONFIRMED FIXED — GYM_CITY_LEADER_EVENT is now derived from STORY_EVENTS_RAW at boot (prior audit 1.3) — `GYM_CITY_LEADER_EVENT` (bug)
- [ISSUE-340] [P3] `isPokeball` flag set on 28 items but never read by the engine — dead metadata — `isPokeball` (data)
- [ISSUE-341] [P3] 9 legacy gen2 berries (Bitter, Burnt, Gold, Ice, Mint, Miracle, Mystery, PRZ Cure, PSN Cure) have no engine handler — `items.json` (data)
- [ISSUE-342] [P3] `delete sm.casinoCoins` cleanup runs on every load, not gated by `_loadedVer` — `load` (bug)
- [ISSUE-343] [P3] CONFIRMED CLEAN — full migrate chain v8→v21 round-trips pre-v15 saves without crash or party/PC/badge loss — `load` (bug)
- [ISSUE-344] [P3] Engine cold-boot is 2.88 s in jsdom — within the harness target (5 s) but **14× the mandate's 200 ms target** — `loadEngine` (perf)
- [ISSUE-345] [P3] Test-harness docs promise window.SAVE_VER / window.sm / window.newStoryRun but only StoryMode + __storyLoad are exposed — `loadEngine` (dx)
- [ISSUE-346] [P3] `console.log` cluster in battle.html — debug noise in shipped code — `loadGameData` (dx)
- [ISSUE-347] [P3] Cold boot is ~3.0 s in jsdom (5-process median 3009 ms) — within the harness's relaxed 5 s self-target but 15× the mandate's 200 ms; a target-mismatch to resolve, not a regression — `loadGameData` (perf)
- [ISSUE-348] [P3] Grade badge prefix differs — `G{tier}` on draft cards vs `T{grade}` on swap/daycare slots — `makeActionBtn` (inconsistency)
- [ISSUE-349] [P3] Button label patterns inconsistent — Professor uses verb+noun, all others noun-only; Evolution facility has 3 names — `makeActionBtn` (inconsistency)
- [ISSUE-350] [P3] Empty-state copy varies across facilities for the same "no party member" condition — `makeActionBtn` (inconsistency)
- [ISSUE-351] [P3] 7 build abilities (Telepathy/Mountaineer/Friend Guard/Healer/Pickup/Rebound/Symbiosis) are silent no-ops the engine never implements — `makeBuild` (data)
- [ISSUE-352] [P3] `makeBuild` is flat across power tiers (T1-T4 spread <0.04 ms median) — confirms no per-tier pathology; baseline 0.045 ms median — `makeBuild` (perf)
- [ISSUE-353] [P3] v15 stable-ID migration only iterates `sm.team`, not `sm.pcBox` (harmless today but inconsistent with v17) — `migrateStoryPreV15` (dx)
- [ISSUE-354] [P3] Pre-v15 saves get 0 Poké Balls instead of the intended 5 — migrateStoryPreV15 balls default is shadowed by the default sm object — `migrateStoryPreV15` (bug)
- [ISSUE-355] [P3] Catch-tutorial migration hard-codes intro-rival index (>1) instead of deriving it — `migrateStoryPreV16` (inconsistency)
- [ISSUE-356] [P3] catchTutorialDone migration hardcodes eventIndex>1 instead of deriving the intro-rival row — `migrateStoryPreV16` (dx)
- [ISSUE-357] [P3] catchTutorialDone migration hard-codes `eventIndex > 1` instead of deriving the intro-rival array index — `migrateStoryPreV16` (dx)
- [ISSUE-358] [P3] Online Host/Join form labels are not programmatically associated with their inputs — `modal-online-host` (a11y)
- [ISSUE-359] [P3] STORY_MODE_FLOW §14d describes Mystery Figure's 7-identity flow + Caged God repurpose; code has only the_first — `MYSTERY_FIGURE_IDENTITIES` (inconsistency)
- [ISSUE-360] [P3] CONFIRMED FIXED — Mystery Figure is now a rotating 10-identity cast (prior audit: hardcoded Cyrus) — `MYSTERY_FIGURE_IDENTITIES` (inconsistency)
- [ISSUE-361] [P3] `mysteryBias` per-variant config is orphaned — seeds weights for retired MF identities, never read — `MYSTERY_FIGURE_IDENTITIES` (data)
- [ISSUE-362] [P3] Mystery Figure rotating-cast scaffolding is now vestigial — collapsed to a single hard-locked identity "the_first" — `MYSTERY_FIGURE_IDENTITIES` (refactor)
- [ISSUE-363] [P3] Fresh runs start with 0 Poké Balls; spec §1/§10 say 5 (only migrated saves get 5) — `newStoryRun` (inconsistency)
- [ISSUE-364] [P3] A cluster of form controls lack accessible names (online host/join, casino bet, gauntlet opt-in) — `online-host-format` (a11y)
- [ISSUE-365] [P3] `parseCSV` of `data/builds.csv` (2.6 MB, 17,397 rows) blocks the main thread for **180 ms median** during boot — `parseCSV` (perf)
- [ISSUE-366] [P3] `parseMoveEffects` per-move latency varies by ~315× between fastest and slowest moves; outliers are 25–250× the median — `parseMoveEffects` (perf)
- [ISSUE-367] [P3] `parseMoveEffects` per-move latency varies ~257× (median 0.012 ms, slowest 3.19 ms) — multi-stat-boost / "dance" moves are the outliers — `parseMoveEffects` (perf)
- [ISSUE-368] [P3] `parseMoveEffects` re-allocates 19 constant `Set` literals on every call and pays a 2–14 ms one-time JIT cost on first touch of each branch (warm cost is fine at ~0.01 ms) — `parseMoveEffects` (perf)
- [ISSUE-369] [P3] parseMoveEffects per-move spread is 130x (stat-stage moves ~1.3ms vs 0.01ms median) — benign, multiple changeStage calls — `parseMoveEffects` (perf)
- [ISSUE-370] [P3] Burn applied as final-damage multiplier (`modifier *= 0.5`) instead of halving the attack stat pre-floor — `parseMoveEffects-burn-modifier` (inconsistency)
- [ISSUE-371] [P3] Stat-change status moves (Decorate, Coaching, Baby-Doll Eyes, Calm Mind) are 50–100× slower than damage moves — `changeStage` calls `logMsg` 1–3× per stat tick, each hitting the 903-key tooltipDict scan — `parseMoveEffects-changeStage-tooltipScan` (perf)
- [ISSUE-372] [P3] Hand-quantification of parseMoveEffects Set-literal churn — 18 `new Set([...])` per call, 13 μs of pure allocation per call (≈70% of warm 18 μs median) — `parseMoveEffects-sets-warm` (perf)
- [ISSUE-373] [P3] Several status moves have no observable effect in the battle engine — `Power Shift` (bug)
- [ISSUE-374] [P3] 129 `@keyframes` definitions but only 5 `prefers-reduced-motion` overrides — sprite-in, dialog-in, master-pulse, badge-pulse all unguarded — `prefers-reduced-motion` (a11y)
- [ISSUE-375] [P3] proceedToNextBattle guards on total team length, but the launch path guards on non-egg fighter count — an all-egg party advances eventIndex then bounces — `proceedToNextBattle` (bug)
- [ISSUE-376] [P3] proceedToNextBattle "no Pokémon" guard counts eggs (team.length) while the fight launch counts only fighters — egg-only party advances then bounces — `proceedToNextBattle` (inconsistency)
- [ISSUE-377] [P3] Mystery-mode Accept has no double-submit guard (re-renders swap picker on repeat clicks) — `profAccept` (bug)
- [ISSUE-378] [P3] Room codes use `Math.random()` over 32 chars × 6 positions — 30 bits, birthday collision at ~30K concurrent rooms, retry-on-23505 saves correctness but not enumeration cost — `randomCode` (bug)
- [ISSUE-379] [P3] README calls shipped catch / PC / Underground / Safari / boss-arc systems "upcoming" — `README.md` (dx)
- [ISSUE-380] [P3] Nature Rater availability is gappy (C0, C3, C5–C9) — absent C1/C2/C4 with no unlock rationale — `renderCityActions` (balance)
- [ISSUE-381] [P3] Rival-gate tip labelled "Heal …" deep-links to the Poké Center, which performs no heal — `renderCityActions` (dx)
- [ISSUE-382] [P3] Dead `'Cyrus'` Mystery-Figure sprite fallbacks remain after the identity was collapsed to a single value ('the_first' / Red) — `renderCityActions` (refactor)
- [ISSUE-383] [P3] Party count chip shows "(N/6)" regardless of the actual badge-driven cap — `renderTeamPanel` (bug)
- [ISSUE-384] [P3] CONFIRMED FIXED — RIVAL_ATTACK_TYPE_DECAY is now 10 (prior audit 1.2 had ÷30 too-aggressive) — `RIVAL_ATTACK_TYPE_DECAY` (balance)
- [ISSUE-385] [P3] Mystery Figure intro pool fallback uses 2 lines but the identity's `intros` field has 4 — falls back silently if the identity object lacks `intros` — `rollMysteryFigureFinalBossTeam` (bug)
- [ISSUE-386] [P3] Mystery Figure climax boss has ZERO gimmicks if the player disabled all 4 mechanics at run start — the "force all on" ctx is dead-coded — `rollMysteryFigureFinalBossTeam` (inconsistency)
- [ISSUE-387] [P3] `rollTrainerTeam` cold-call is **1.63 ms median (max 3.22 ms)**; well under the 50 ms target but worth recording as the deep-dive baseline before the upcoming difficulty-curve work — `rollTrainerTeam` (perf)
- [ISSUE-388] [P3] Safari flee-risk surfaced in color (orange #ff7043) and small 10px text only — `safariActionRow` (contrast)
- [ISSUE-389] [P3] Move-test generator strips apostrophes, and the engine silently runs unknown move names as a 187-dmg fallback — `safeName` (dx)
- [ISSUE-390] [P3] SAVE_VER=23 but migration chain stops at `_loadedVer < 22` — no migrateStoryPreV23, no boot shadow-validation — `SAVE_VER` (dx)
- [ISSUE-391] [P3] All 24 `#screen-*` containers are plain `<div>` — no `<main>` or `role="region"` — `screen-landmarks` (a11y)
- [ISSUE-392] [P3] Several story-mode dev seeds use `Math.random` for build / sprite picks, breaking seeded replays when debug seeds are in play — `seedDebugMysteryLegendGate` (bug)
- [ISSUE-393] [P3] End-of-turn residual logic is duplicated verbatim in forced-switch path and main loop — divergence risk — `selectPartyMember` (inconsistency)
- [ISSUE-394] [P3] Player gimmick-unlock order shifts when a mechanic is disabled in settings — DMax unlocks at Gym 5 instead of Gym 6 if Mega is off — `settings.megaOn` (dx)
- [ISSUE-395] [P3] `shouldForceCityProfessor` uses dead `sm.team.length < 6` floor that's shadowed by outer `hasTeamRoom` gate — `shouldForceCityProfessor` (refactor)
- [ISSUE-396] [P3] `shouldForceCityProfessor`'s `sm.team.length < 6` floor is dead-coded once the outer `hasTeamRoom` gate clamps to `_storyMaxPartySize()` — `shouldForceCityProfessor` (dx)
- [ISSUE-397] [P3] Stale comment claims rival secondary intro "Uses Math.random" — it now uses seeded _storySideRng — `showBattleIntro` (inconsistency)
- [ISSUE-398] [P3] Mobile move-details "i" is a span[role=button] with no tabindex/keydown — keyboard cannot reach it — `showMoves` (a11y)
- [ISSUE-399] [P3] Victory overlay auto-dismisses after 6s regardless of how much narrative it stacks — the biggest story beats can vanish before they're read — `showVictoryOverlay` (dx)
- [ISSUE-400] [P3] Subject Zero stored to PC (party-full at cage) shows "Subject Zero" nickname but is never auto-fielded — easy to miss the capstone mon — `showVictoryOverlay` (inconsistency)
- [ISSUE-401] [P3] Inconsistent auto-dismiss across scene types — victory 6s timeout vs beat scenes never auto-dismiss — `showVictoryOverlay` (inconsistency)
- [ISSUE-402] [P3] Doc battle.html:LINE anchors are stale (23/43 drifted across the spec docs) — `spec-drift-doc-anchors` (dx)
- [ISSUE-403] [P3] Mystery Figure identity is rolled at run start before sm.active/runSeed are live — not reproducible under fixed debug seeds — `startNewRun` (bug)
- [ISSUE-404] [P3] Row-67 `STORY_BEATS` still tags `'cagedGod'` + coldOpen `mystery67` — stale cut-arc residue in the live beat map — `STORY_BEATS` (dx)
- [ISSUE-405] [P3] Internal action keys use `Pokemon` (no diacritic) while UI labels use `Pokémon` — `STORY_EVENTS_RAW` (inconsistency)
- [ISSUE-406] [P3] Doc line anchors stale — 18/50 `battle.html:LINE` refs in design docs no longer resolve (clustered) — `STORY_EVENTS_RAW` (dx)
- [ISSUE-407] [P3] Doc `battle.html:LINE` anchors stale — 18/50 references drifted (cluster) — `STORY_EVENTS_RAW` (dx)
- [ISSUE-408] [P3] Doc `battle.html:LINE` anchors stale in surviving specs — 24/44 drifted (post-cleanup cluster; updates ISSUE-330) — `STORY_EVENTS_RAW` (dx)
- [ISSUE-409] [P3] Service-timeline pacing — City1 post-gym hub is a dead zone (no new "thing to do") — `STORY_EVENTS_RAW` (balance)
- [ISSUE-410] [P3] STORY_EVENTS_RAW has 67 array rows (incl. 2 'Hall of Fame' string matches) — mandate/spec cite "68 rows" — `STORY_EVENTS_RAW` (data)
- [ISSUE-411] [P3] STORY_EVENTS_RAW resolves to 67 rows in harness vs 68 stated in spec/mandate — `STORY_EVENTS_RAW` (data)
- [ISSUE-412] [P3] Timeline is 67 rows; STORY_MODE_FLOW.md (and this update's brief) still say "68 rows" — `STORY_EVENTS_RAW` (inconsistency)
- [ISSUE-413] [P3] STORY_MODE_FLOW §4 still specs the flat Safari weights g1:3/g2:22/g3:50/g4:25; live code is a badge curve (_SAFARI_GRADE_CURVE_BY_BADGES) — `STORY_EVENTS_RAW` (inconsistency)
- [ISSUE-414] [P3] `STORY_IV_TIER_RANGES` is dead — superseded by `STORY_IV_TIER_CENTER`, zero consumers — `STORY_IV_TIER_RANGES` (refactor)
- [ISSUE-415] [P3] THREE story IV tables coexist — `STORY_IV_TIER_RANGES` is fully dead; `STORY_IV_TIER_CENTER` is Frontier-only; only `STORY_IV_CITY_*` is the live story curve — `STORY_IV_TIER_RANGES` (inconsistency)
- [ISSUE-416] [P3] docs/STORY_MODE_AUDIT.md is stale — most of its flagged issues are now fixed (SAVE_VER 14→22) — `STORY_MODE_AUDIT` (dx)
- [ISSUE-417] [P3] 6+ stale `battle.html:LINE` refs in STORY_MODE_CATCH_INTEGRATION_RISK.md (avg drift ~9000 lines) — `STORY_MODE_CATCH_INTEGRATION_RISK.md` (dx)
- [ISSUE-418] [P3] Doc `battle.html:LINE` anchors still stale (50 refs, 18 drifted) despite PR #140 "fix" — `STORY_MODE_CATCH_INTEGRATION_RISK.md` (dx)
- [ISSUE-419] [P3] 9 of 10 `battle.html:LINE` refs in STORY_MODE_FLOW.md are stale (avg drift ~7000 lines) — `STORY_MODE_FLOW.md` (dx)
- [ISSUE-420] [P3] STORY_MODE_FLOW.md PC cap of 10 contradicts shipped PC_BOX_CAP=30 — `STORY_MODE_FLOW.md` (data)
- [ISSUE-421] [P3] Doc line anchors stale across 4 specs (still drifting post-v24; cluster) — `STORY_NARRATIVE_VARIANTS` (dx)
- [ISSUE-422] [P3] 5+ stale `battle.html:LINE` refs in STORY_NARRATIVE_VARIANTS.md (variant-system anchors moved 3-5k lines) — `STORY_NARRATIVE_VARIANTS.md` (dx)
- [ISSUE-423] [P3] Spec/mandate says timeline is "68 rows"; STORY_EVENTS_RAW has 67 (array idx 0–66), and rowId 68 is the intro Rival at array idx 1 — `STORY_RIVAL_ROW_INTRO` (inconsistency)
- [ISSUE-424] [P3] Tutorial scenes are text-only walls of 60-120 words — no audio, no progressive reveal, no skip-to-end affordance — `STORY_TUTORIAL_SCENES` (dx)
- [ISSUE-425] [P3] Catch ball buttons lack accessible name reading the success %; bare-icon img alt empty — `story-catch-ball` (a11y)
- [ISSUE-426] [P3] Crucible & Catch headers use empty spacer spans instead of a back control; no escape from Crucible header — `story-crucible-header` (a11y)
- [ISSUE-427] [P3] Pokédex counts strip updates live (seen/caught) but is not an aria-live region — `story-pc-pokedex-strip` (a11y)
- [ISSUE-428] [P3] Dead CSS selector for a #story-pc-tab-journal-btn that has no markup or handler — `story-pc-tab-journal-btn` (refactor)
- [ISSUE-429] [P3] Pokémon Center tab buttons not exposed as a tablist (no role=tab/tabpanel) — `story-pc-tab-storage-btn` (a11y)
- [ISSUE-430] [P3] Mobile touch targets in story shops top out at 42px — under the WCAG 44×44 baseline — `story-shop-buy-btn` (a11y)
- [ISSUE-431] [P3] Tutorial overlay's four-stage entrance animation has no reduced-motion fallback — `story-tutorial-overlay` (a11y)
- [ISSUE-432] [P3] Scope-leak audit (same class as fixed `sm` bug) — NEGATIVE: no other IIFE-internal symbol referenced bare from turn-loop scope — `storyAwareRng` (inconsistency)
- [ISSUE-433] [P3] `storyAwareRng()` helper exists but the combat engine still calls bare `Math.random()` at ~250 sites — `storyAwareRng` (dx)
- [ISSUE-434] [P3] Master Ball purple pulse runs forever with no reduced-motion override; surrounding text/glow stays readable but the loop is hostile — `storyCatchMasterPulse` (a11y)
- [ISSUE-435] [P3] CONFIRMED FIXED — Hard coin mult floored to 1.00 (prior audit 2.1); Challenge 1.10 — `storyDifficultyCoinMult` (balance)
- [ISSUE-436] [P3] Hard mode still earns less gold per fight than Normal (1.00 vs 1.30) despite facing 1.15x-stronger foes — residual difficulty/economy asymmetry — `storyDifficultyCoinMult` (balance)
- [ISSUE-437] [P3] `storyRngNext` / per-engine-entry `rng` ternary accessor is **0.0003 ms (300 ns) per call** — no closure allocation penalty, accessor pattern is clean — `storyRngNext` (perf)
- [ISSUE-438] [P3] Status indicator toggles role="status" + aria-label onto a <div> at content-mutation time — `updateUI` (a11y)
- [ISSUE-439] [P3] Upper Hand / Shell Trap don't enforce their precondition gate — `Upper Hand` (bug)
- [ISSUE-440] [P3] `sm.wildSeenByEventIdx` and `sm.staticDrops` are lazily initialized instead of declared in the `sm` defaults block — `wildSeenByEventIdx` (dx)
- [ISSUE-441] [P4] VERIFIED OK — boss/raid reward double-grant across the two call sites is structurally prevented — `_activeBattleBeatForCurrentRow` (bug)
- [ISSUE-442] [P4] `_bossWeatherLocked` / `_bossTerrainLocked` flags are set but never read — field "lock" is opening-state only — `_storyBossMechanicsBattleInit` (refactor)
- [ISSUE-443] [P4] VERIFIED OK — extra-arc raid "laid to rest, no catch" lock is enforced structurally — `enterCatchEncounter` (bug)
- [ISSUE-444] [P4] `SAVE_VER = 23` but migration dispatch stops at `_loadedVer < 22` — no numbered v23 step — `SAVE_VER` (dx)
- [ISSUE-445] [P4] Three redundant RNG-routing mechanisms; confusion self-hit relies solely on the global Math.random patch while siblings use storyAwareRng() — `storyAwareRng` (inconsistency)
...
## <a id="ISSUE-019"></a> ISSUE-019: Late-game enemy weak-mon floor + endgame gold over-accumulation (2026-06 balance pass) — IMPLEMENTED
...
## <a id="ISSUE-020"></a> ISSUE-020: Villain-boss Master Ball grant has no fire-once guard; unique-ball guarantee can break
...
## <a id="ISSUE-021"></a> ISSUE-021: Two Master Ball sources collide — villain-track boss (Road 7, pre-HoF) + post-HoF broker = 2 per run
...
## <a id="ISSUE-022"></a> ISSUE-022: Road event-beats fire before in-city Gym Trainer / Gym Leader fights, not only on the route
...
## <a id="ISSUE-023"></a> ISSUE-023: Player gimmick gate reads bare IIFE-private `sm` — always sees zero unlocked gimmicks
...
## <a id="ISSUE-024"></a> ISSUE-024: aiDecision early-returns null on any choiceLock — AI cannot switch out of an immune/walled lock
...
## <a id="ISSUE-025"></a> ISSUE-025: Difficulty tiers scale only enemy stats; AI policy is byte-identical at every tier — no rising *challenge*, just a rising stat-wall
...
## <a id="ISSUE-026"></a> ISSUE-026: Measured curve violates the "regular<player, gym slightly-above, E4 EQUAL" intent — GL1-2 are 0.67-0.83× the player, gyms overshoot to 1.5×, E4 is ~1.70× (not equal)
...
## <a id="ISSUE-027"></a> ISSUE-027: Sleep duration roll uses bare `Math.random()` (RNG drift on every sleep proc)
...
## <a id="ISSUE-028"></a> ISSUE-028: League foe stat boost stacks multiplicatively despite comment claiming additive merge
...
## <a id="ISSUE-029"></a> ISSUE-029: Black Market shop from STORY_FEATURES_INTEGRATION.md §3 is still entirely unimplemented
...
## <a id="ISSUE-030"></a> ISSUE-030: Three conflicting "canon" docs for the boss/endgame arc; code matches none cleanly
...
## <a id="ISSUE-031"></a> ISSUE-031: `canMove` paralysis + confusion self-hit checks use bare `Math.random()` (RNG drift in story replays)
...
## <a id="ISSUE-032"></a> ISSUE-032: Sleep wake-check is off-by-one — ~1/3 of sleeps cost 0 turns (instant wake, mon acts same turn)
...
## <a id="ISSUE-033"></a> ISSUE-033: Sleep off-by-one: sleepDuration=1 wakes and attacks on its first turn (0 turns lost); effective sleep is 0-2 turns not 1-3
...
## <a id="ISSUE-034"></a> ISSUE-034: `canMove` paralysis fizzle uses bare `Math.random()` while sibling freeze thaw uses `storyRngNext`
...
## <a id="ISSUE-035"></a> ISSUE-035: Casino Coin Flip outcome uses Math.random(), not seeded storyRngNext()
...
## <a id="ISSUE-036"></a> ISSUE-036: Casino Roulette winning cell chosen with Math.random(), not seeded RNG
...
## <a id="ISSUE-037"></a> ISSUE-037: Ball inventory + `catchMode` flag in STORY_FEATURES_INTEGRATION §1/§2/§5 do not match shipped code
...
## <a id="ISSUE-038"></a> ISSUE-038: The unique Master Ball can be wasted on any wild — soft-locks the Caged God capture (only 1% catch rate without it)
...
## <a id="ISSUE-039"></a> ISSUE-039: Unique Master Ball can be wasted on any non-boss wild (Crucible wild encounter), soft-locking the Caged God capture — VERIFIED still present post-merge
...
## <a id="ISSUE-040"></a> ISSUE-040: Toxic (badly-poison) counter `statusTurns` is not reset on switch-out
...
## <a id="ISSUE-041"></a> ISSUE-041: Crucible League Run skips E1 and ends on the Rival — `_CRUCIBLE_LEAGUE_ROWS` are off-by-one row-ids
...
## <a id="ISSUE-042"></a> ISSUE-042: proceedToNextBattle re-entry stacks duplicate cold-open overlays, wedging progression (the "After Badge One" stuck state)
...
## <a id="ISSUE-043"></a> ISSUE-043: Post-HoF Crucible hub button gated on dead `bossArc.available` — never renders
...
## <a id="ISSUE-044"></a> ISSUE-044: Retune risk: `data/builds/gen*.json` is a fallback mirror, not the live build source (`data/builds.csv` is authoritative)
...
## <a id="ISSUE-045"></a> ISSUE-045: Choice-locked AI re-returns the locked move with zero immunity/wall check — spams 0-dmg moves forever
...
## <a id="ISSUE-046"></a> ISSUE-046: AI spams setup move into an active phazer — `score *= 0.25` penalty loses to near-zero attack scores
...
## <a id="ISSUE-047"></a> ISSUE-047: Illegal Dealer NPC encounter (STORY_FEATURES_INTEGRATION.md §3.5) still missing
...
## <a id="ISSUE-048"></a> ISSUE-048: City-8 legendary Mystery gate is bypassed if party has < 6 members
...
## <a id="ISSUE-049"></a> ISSUE-049: Full Itinerary scaffolding (STORY_FEATURES_INTEGRATION.md §10) still entirely unimplemented
...
## <a id="ISSUE-050"></a> ISSUE-050: `lastRemoteSeq` is updated *before* the remote handler runs — a thrown handler still bumps the watermark, so the failed update is permanently skipped
...
## <a id="ISSUE-051"></a> ISSUE-051: Save-migration integration test never exercises the migrate chain (vacuous pass)
...
## <a id="ISSUE-052"></a> ISSUE-052: v14 trainer-assignment migration is bundled under `_loadedVer < 13`, so v13 saves skip it
...
## <a id="ISSUE-053"></a> ISSUE-053: Secondary flinch/Stench write `defender.volatile.flinch` unguarded — throws if volatile missing (sibling _tryConfuse guards it)
...
## <a id="ISSUE-054"></a> ISSUE-054: Many `parseMoveEffects` branches still use bare `Math.random()` — seeded story replays drift
...
## <a id="ISSUE-055"></a> ISSUE-055: Data-driven boost block returns early, bypassing named-branch extra effects (Memento self-faint, Toxic Thread poison)
...
## <a id="ISSUE-056"></a> ISSUE-056: Core damage roll, crit, and accuracy use bare `Math.random()` — the highest-impact drift sites
...
## <a id="ISSUE-057"></a> ISSUE-057: Damage formula divides un-truncated (fractional) A/D — Showdown floors atk/def stats first (±1 HP)
...
## <a id="ISSUE-058"></a> ISSUE-058: Damage roll is continuous `0.85+rand*0.15` — never reaches 100%, so max-roll damage is unreachable
...
## <a id="ISSUE-059"></a> ISSUE-059: Eight contact-triggered defender ability procs (Static / Poison Point / Flame Body / Cute Charm / Effect Spore / Toxic Chain / Cursed Body / Poison Touch) all use bare `Math.random()`
...
## <a id="ISSUE-060"></a> ISSUE-060: Contact-ability procs (Static / Poison Point / Flame Body / Cute Charm / Poison Touch / Toxic Chain / Cursed Body) all bare `Math.random()`
...
## <a id="ISSUE-061"></a> ISSUE-061: PC_BOX_CAP is 30 in code but the canonical spec says 10
...
## <a id="ISSUE-062"></a> ISSUE-062: Battle for Pokémon wager system (STORY_FEATURES_INTEGRATION.md §6) still missing
...
## <a id="ISSUE-063"></a> ISSUE-063: Fire-type damaging moves do not thaw a frozen target (only flag-marked moves thaw)
...
## <a id="ISSUE-064"></a> ISSUE-064: HP-restore berry (Sitrus/Oran) eaten mid-hit suppresses Berserk / Wimp Out / Anger Shell HP-cross
...
## <a id="ISSUE-065"></a> ISSUE-065: Multi-hit contact moves skip all on-contact abilities/items (Rough Skin, Iron Barbs, Rocky Helmet, Static, etc.)
...
## <a id="ISSUE-066"></a> ISSUE-066: Multi-hit moves skip the Shield Dust / Sheer Force / Covert Cloak / Substitute secondary gate
...
## <a id="ISSUE-067"></a> ISSUE-067: Solar Beam bad-weather power halving is dead code — checks `"SolarBeam"` (no space) which never matches
...
## <a id="ISSUE-068"></a> ISSUE-068: Future Sight / Doom Desire resolve one turn too early (set to 2 turns; spec & Showdown require a 2-turn delay = 3)
...
## <a id="ISSUE-069"></a> ISSUE-069: Turn-order priority breakers (Quick Claw, speed-tie, rampage duration) use bare `Math.random()`
...
## <a id="ISSUE-070"></a> ISSUE-070: End-of-turn residual block is not try-wrapped — any throw masks as "Turn skipped" + skips residuals
...
## <a id="ISSUE-071"></a> ISSUE-071: `pushData` queue keeps advancing after a write fails — `_pushDataImpl` errors are swallowed to `console.warn`, leaving local state diverged from Supabase
...
## <a id="ISSUE-072"></a> ISSUE-072: `pvp_rooms` SELECT policy is `using (true)` — anyone with the public key can scrape every live match's full draft + battle state
...
## <a id="ISSUE-073"></a> ISSUE-073: `remoteRowQueue` has no timeout — a hung `onOnlineRoomData` (e.g., a UI promise that never resolves) freezes ALL future remote updates
...
## <a id="ISSUE-074"></a> ISSUE-074: Player-facing city actions read "Pokemon League" / "Pokemon Fan Club" (no diacritic)
...
## <a id="ISSUE-075"></a> ISSUE-075: Draft pick cards are click-only <div>s — keyboard/SR users cannot select a Pokémon
...
## <a id="ISSUE-076"></a> ISSUE-076: `No Item` sentinel string used in 11 build slots is absent from `data/items.json`
...
## <a id="ISSUE-077"></a> ISSUE-077: Battle log (#battle-log) only cleared on returnToHome, not at battle start; previous fight's lines bleed in
...
## <a id="ISSUE-078"></a> ISSUE-078: showScreen() does no focus management on story-screen transitions — focus is orphaned
...
## <a id="ISSUE-079"></a> ISSUE-079: Mechanics-unlock docs still describe one-per-gym drip; code now unlocks all four at Colress/City 6
...
## <a id="ISSUE-080"></a> ISSUE-080: Boss-mechanic hookup reads window.StoryMode.{BOSS_CONFIGS,bossMechanics*} but those live on test-only __storyTest — boss arc still dead in prod
...
## <a id="ISSUE-081"></a> ISSUE-081: Boss/raid mechanics state never reset; bleeds into next ordinary Story fight
...
## <a id="ISSUE-082"></a> ISSUE-082: Fresh run starts with 0 PokéBalls (spec says 5 at run start); 5 are gifted at first Mart instead
...
## <a id="ISSUE-083"></a> ISSUE-083: Three mutually-incompatible story-narrative designs coexist; no doc is the single canon
...
## <a id="ISSUE-084"></a> ISSUE-084: Reaper's Toll uses bare IIFE-private `storyRngNext` — guard always false, breaks seeded replays
...
## <a id="ISSUE-085"></a> ISSUE-085: Pokémon Trader (STORY_FEATURES_INTEGRATION.md §7) at City4 still missing
...
## <a id="ISSUE-086"></a> ISSUE-086: Speed-tie resolution + Quick Claw + lock-turn duration all use bare `Math.random()` in the turn loop
...
## <a id="ISSUE-087"></a> ISSUE-087: `???` type used by gen1 `bide` and gen4 `curse` is not defined in `typeChart`
...
## <a id="ISSUE-088"></a> ISSUE-088: Turn-resolution catch masks any in-loop throw as "[Error: …. Turn skipped.]" — both moves abandoned, real bugs hidden (PT-001)
...
## <a id="ISSUE-089"></a> ISSUE-089: ~250-line Caged God boss arc is dead code (unreachable) but still fully shipped
...
## <a id="ISSUE-090"></a> ISSUE-090: Caged God lead spec (§9 "visit Cities 2/5/8") contradicts shipped Crucible-hub collection; §14b omits the arc
...
## <a id="ISSUE-091"></a> ISSUE-091: Post-game lead "hunt" collapses to 3 buttons on one Crucible screen — no travel, no gating
...
## <a id="ISSUE-092"></a> ISSUE-092: Extra-track raid HP comment says miniRaid=(party-2)×, raid=(party-1)× base HP, but a separate ×1.3 _bossStatMult also multiplies maxHp — true HP is ~5.2×/6.5× at party 6
...
## <a id="ISSUE-093"></a> ISSUE-093: Two parallel story-flow engines coexist — new "unified" engine built but never wired (P2/P3 never done)
...
## <a id="ISSUE-094"></a> ISSUE-094: Dormant "unified flow engine" is now triple-orphaned — live dispatch is a 3rd design that leapfrogged it
...
## <a id="ISSUE-095"></a> ISSUE-095: Variants are rolled every run (not forced classic) — so variant Champion/post-HoF lines pointing at the dead broker/cage DO fire
...
## <a id="ISSUE-096"></a> ISSUE-096: Colress Signature-Z silently overwrites the last move; confirm warns only about item/gimmick
...
## <a id="ISSUE-097"></a> ISSUE-097: Nature Rater cost badge shows "2000+" but TUTOR_COST_NATURE is a flat 2000
...
## <a id="ISSUE-098"></a> ISSUE-098: 7 duplicated `select('data').eq('id', roomId).single()` fetch+error blocks in online-pvp.js
...
## <a id="ISSUE-099"></a> ISSUE-099: Build "illegal-ability" pairs (672) are intended (hackmons/AAA + mega-form abilities); a naive legality check false-positives
...
## <a id="ISSUE-100"></a> ISSUE-100: species.json Hisui formes are stale (gen8 snapshot) — Samurott-Hisui/Kleavor lack gen9 Sharpness, so every legal-tier build is dropped
...
## <a id="ISSUE-101"></a> ISSUE-101: Cable Link gimmick gate contradicts CHANGELOG promise that Cable Link surfaces pre-unlock mechanics
...
## <a id="ISSUE-102"></a> ISSUE-102: Story autosave "💾 Saved" toast bypasses the polite live region — screen readers miss it
...
## <a id="ISSUE-103"></a> ISSUE-103: "PC nearly full" warning threshold is `PC_BOX_CAP - 3` (27/30) but the spec calls for 80% (8/10)
...
## <a id="ISSUE-104"></a> ISSUE-104: PC deposit/withdraw/release resets scroll to top via full innerHTML rewrite
...
## <a id="ISSUE-105"></a> ISSUE-105: Rivalry tab is mis-homed in the Pokémon Center — belongs in a progression/journal surface
...
## <a id="ISSUE-106"></a> ISSUE-106: Underground claims "Starters … aren't for sale" but starters are sellable (unsellable flag stripped)
...
## <a id="ISSUE-107"></a> ISSUE-107: Villain-arc regular battles share 7 generic `tag:'villain'` grunts — never themed to the run's actual track (3 tracks have no matching grunt at all)
...
## <a id="ISSUE-108"></a> ISSUE-108: _renderCrucible rebuilds a 17.7KB / 109-node innerHTML on every open + lead-collect + hard-mode toggle
...
## <a id="ISSUE-109"></a> ISSUE-109: Battle Frontier hub displays stale, weaker foe-scaling numbers than what applyStoryLeagueFoeStatBoost actually applies
...
## <a id="ISSUE-110"></a> ISSUE-110: ~12 parallel event-presentation paths with 3 z-index layers and no single registry
...
## <a id="ISSUE-111"></a> ISSUE-111: Road beat clumping: 2 beats/road (villain road7 = 3, league = 7) play back-to-back, breaking pacing
...
## <a id="ISSUE-112"></a> ISSUE-112: Catch-tutorial gate comment claims "starting kit gives 5 balls" — fresh-run kit is actually 0
...
## <a id="ISSUE-113"></a> ISSUE-113: Story tutorial overlay (STORY_TUTORIAL_SCENES) is not a dialog — no role, focus trap, or ESC
...
## <a id="ISSUE-114"></a> ISSUE-114: WANDER_AROUND_SPEC says "not yet implemented" but Wander Around shipped at SAVE_VER 23
...
## <a id="ISSUE-115"></a> ISSUE-115: Current enemy curve is lumpy, not rising — GL4=GL5 dead zone (foe mult 1.0) and a GL8 quad-cliff (T3->T4 + IV 18->26 + gimmicks 2->3)
...
## <a id="ISSUE-116"></a> ISSUE-116: Boss field-lock sets _bossWeatherLocked/_bossTerrainLocked but nothing reads them; weather decay path (20611/20617) ignores the lock
...
## <a id="ISSUE-117"></a> ISSUE-117: Boss surge/immunity timers live on the active foe mon — lost on switch, stale on bench
...
## <a id="ISSUE-118"></a> ISSUE-118: Single `_bossPendingTelegraph` slot drops a phase when two mechanics telegraph on the same turn (mfBattle)
...
## <a id="ISSUE-119"></a> ISSUE-119: Shipped BOSS_CONFIGS uses surge/immunity/heal phases, not the EXPANSION_PLAN "multi-form transformation"
...
## <a id="ISSUE-120"></a> ISSUE-120: In-catch "Up next" says "trainer/wild" but a battle-kind beat scene fires between catch and fight
...
## <a id="ISSUE-121"></a> ISSUE-121: Foe party size matches `team.length` with per-role floors instead of the spec'd `min(6, 2+badges)` badge curve
...
## <a id="ISSUE-122"></a> ISSUE-122: Caged God removal (v24) is incomplete — residual content/help-text/achievements still reference the cut arc
...
## <a id="ISSUE-123"></a> ISSUE-123: rollTrainerTeam's evo-stage cap uses cityIndexFromEventIndex on a ROW ID (not array index) — intro Rival gets cap 2 (fully evolved) instead of 0 (basics-only)
...
## <a id="ISSUE-124"></a> ISSUE-124: Master Ball granted by BOTH villain-boss victory and post-HoF Caged God, vs spec "1 per run"
...
## <a id="ISSUE-125"></a> ISSUE-125: `_storyGrantTrackEndReward` has no internal idempotency guard — re-call double-grants Master Ball
...
## <a id="ISSUE-126"></a> ISSUE-126: Planned hatch animation + Fight Club transitions need reduced-motion + live-region design up front
...
## <a id="ISSUE-127"></a> ISSUE-127: `_storyPickMysteryIdentity` uses bare `Math.random()` — Mystery Figure roll diverges across seeded replays
...
## <a id="ISSUE-128"></a> ISSUE-128: CSV-load cache invalidation pokes IIFE-private `_txMetaCache`/`_txGlobalMetaCached` — invalidation is dead
...
## <a id="ISSUE-129"></a> ISSUE-129: `_variantMysteryOutro` is dead — `_MYSTERY_OUTRO_BY_VARIANT` keyed only by retired identities, never matches `the_first`
...
## <a id="ISSUE-130"></a> ISSUE-130: All ~30 per-variant Mystery-Figure outros are dead — keyed by retired identities, never match `the_first`
...
## <a id="ISSUE-131"></a> ISSUE-131: FLOW §3 says wild grade is "keyed on sm.badges" via `_WILD_GRADE_CURVE_BY_BADGES`; shipped code keys it on city
...
## <a id="ISSUE-132"></a> ISSUE-132: applyStatus dereferences `state.pActive.volatile.lockMove` / `state.fActive.volatile.lockMove` unconditionally (Uproar check) — throws if an active is null
...
## <a id="ISSUE-133"></a> ISSUE-133: Mystery Figure HP boost is 1.35 in code but 1.50 in both balance docs (canonical-curve drift)
...
## <a id="ISSUE-134"></a> ISSUE-134: Dual-mega stone (Charizard/Mewtwo X vs Y) picked with bare Math.random — breaks seeded replay
...
## <a id="ISSUE-135"></a> ISSUE-135: Team Yell villain track has no themed sprites — boss Piers falls back to generic Roughneck, mini-boss Marnie renders as Gladion
...
## <a id="ISSUE-136"></a> ISSUE-136: `battle`-kind beats (main.battle1/battle2) launch a generic trainer, not a themed one — no canon override
...
## <a id="ISSUE-137"></a> ISSUE-137: Memory growth is benign (linear, ~25 KB/turn, R² = 0.712 — noise-dominated) and **does not indicate a leak**; the mandate's "60 turn / quadratic = P1" threshold is not hit
...
## <a id="ISSUE-138"></a> ISSUE-138: `perf-bench.mjs` calls `parseMoveEffects(move)` with the wrong arg-count; the reported 1.4 ms "median" is the cost of a thrown `TypeError`, not real per-move parsing
...
## <a id="ISSUE-139"></a> ISSUE-139: Turn-loop max latency is **78–84 ms** with IQR 12 ms — within the harness target (50 ms median is OK) but max is 5× the median, indicating a per-turn outlier path
...
## <a id="ISSUE-140"></a> ISSUE-140: Caged God "Key" lead has zero cost — spec says it should demand strongest mon or steep gold
...
## <a id="ISSUE-141"></a> ISSUE-141: buyArtifact has no interaction lock; double-submit safety rests solely on confirm-modal z-order
...
## <a id="ISSUE-142"></a> ISSUE-142: Casino subtitle cream text on light-gold/rose panels fails WCAG AA contrast
...
## <a id="ISSUE-143"></a> ISSUE-143: catch-system integration test asserts a stale "PC cap of 10" and passes on an incidental substring — same hollow-test pattern as safari-zone
...
## <a id="ISSUE-144"></a> ISSUE-144: PC-cap integration test asserts cap 10 (stale) and passes only via false-positive regex match
...
## <a id="ISSUE-145"></a> ISSUE-145: Unique Master Ball is spendable on any regular wild → Caged God capture becomes a 1%-per-throw grind
...
## <a id="ISSUE-146"></a> ISSUE-146: `sm.catchUnlocked` is written 3× but never read; spec §10 says it gates wild-route prompts
...
## <a id="ISSUE-147"></a> ISSUE-147: `sm.settings.catchMode` toggle never implemented; catch shipped as always-on, 3 specs still gate on it
...
## <a id="ISSUE-148"></a> ISSUE-148: autopilot-player classify() treats a cold-open as a city — pump fires city actions instead of dismissing the overlay, masking/causing the stuck-on-"After Badge One" report
...
## <a id="ISSUE-149"></a> ISSUE-149: Comeuppance reflects 0 damage in all cases (twin Metal Burst works)
...
## <a id="ISSUE-150"></a> ISSUE-150: Single Master Ball is a free consumable — spending it pre-cage leaves boss arc as a 1%-per-throw grind
...
## <a id="ISSUE-151"></a> ISSUE-151: Pre-boss-arc post-HoF saves may never receive the Master Ball / boss arc if parked at a city row on load
...
## <a id="ISSUE-152"></a> ISSUE-152: Crucible "Mystery Figure" rematch uses out-of-bounds index 67 (array length is 67, max idx 66)
...
## <a id="ISSUE-153"></a> ISSUE-153: Crucible "Rival Rematch" targets the Hall of Fame row (array idx 65), not the league rival
...
## <a id="ISSUE-154"></a> ISSUE-154: Crush Grip doesn't scale with target HP (constant ~2 dmg); siblings do
...
## <a id="ISSUE-155"></a> ISSUE-155: `deepClone` falls back to `JSON.parse(JSON.stringify(...))` — silently drops Set/Map/Date/undefined/circular refs across the entire snapshot pipeline
...
## <a id="ISSUE-156"></a> ISSUE-156: Design checklist's load-bearing "CSS block = battle.html lines 16-4156" guardrail is wrong by ~3700 lines
...
## <a id="ISSUE-157"></a> ISSUE-157: Malva (Kalos E1) has a victory line but no intro pool in `TRAINER_QUOTES_BY_NAME`
...
## <a id="ISSUE-158"></a> ISSUE-158: Leech Seed end-of-turn drain ignores Magic Guard (holder loses HP, seeder heals)
...
## <a id="ISSUE-159"></a> ISSUE-159: Partial-trap (Bind / Fire Spin / Whirlpool / Sand Tomb) end-of-turn damage ignores Magic Guard
...
## <a id="ISSUE-160"></a> ISSUE-160: Relic vs Artifact used interchangeably for one object across label/key/fn/state
...
## <a id="ISSUE-161"></a> ISSUE-161: PLAN COLLISION — Daycare unlock is keyed on the "Gym Leader 1" event name, not a city; redesign wants C2/C4/C6
...
## <a id="ISSUE-162"></a> ISSUE-162: Poké Center never clears Fatigue, yet the in-game bulletin tells players a Center stay clears it
...
## <a id="ISSUE-163"></a> ISSUE-163: Validator reports 6925 "CSV alternative" occurrences but every build file uses arrays — false positive
...
## <a id="ISSUE-164"></a> ISSUE-164: Facility debut cities disagree 3 ways (code FACILITY_DEBUT_CITY vs both balance docs vs in-code comments)
...
## <a id="ISSUE-165"></a> ISSUE-165: Early-game softening is a per-CITY table in code, not the per-event named constants both balance docs describe
...
## <a id="ISSUE-166"></a> ISSUE-166: No sleep clause in story + AI scores Spore at 100 → up to 3-turn lock loops, amplified by high-tier stat bloat (fairness risk)
...
## <a id="ISSUE-167"></a> ISSUE-167: When every damaging move is immune (score 0), AI throws a 0-dmg attack instead of switching/using status
...
## <a id="ISSUE-168"></a> ISSUE-168: Salac Berry grants a phantom 1.5x Speed while merely held at <=25% HP (not consumed)
...
## <a id="ISSUE-169"></a> ISSUE-169: City-3 HUD/route name falls back to "City 3" — GYM_CITY_LEADER_EVENT array-index keys trainerAssignments (row-id keyed)
...
## <a id="ISSUE-170"></a> ISSUE-170: 36 references to 18 distinct `global.__*` variables across `online-pvp.js` — the PvP module mutates host-side state through ad-hoc globals instead of a single observable
...
## <a id="ISSUE-171"></a> ISSUE-171: City-3 display name always falls back to "City 3" — GYM_CITY_LEADER_EVENT returns an array index, but trainerAssignments is keyed by row ID
...
## <a id="ISSUE-172"></a> ISSUE-172: In-game Help "Catching" section still points players to the cut Caged God arc
...
## <a id="ISSUE-173"></a> ISSUE-173: City-8 "Required" legendary handoff silently downgrades to a normal Professor gift when the party is below cap
...
## <a id="ISSUE-174"></a> ISSUE-174: `_loadedVer < 14` block missing — v14 trainer-name migration never runs for saves stamped at exactly v13
...
## <a id="ISSUE-175"></a> ISSUE-175: Migration chain is sound but unobservable — no boot-time shadow validation
...
## <a id="ISSUE-176"></a> ISSUE-176: Pre-merge saves with partial unlockedGimmicks are not re-derived on load — Tera/Z silently withheld until next milestone win
...
## <a id="ISSUE-177"></a> ISSUE-177: No save migration coerces stale `sm.mysteryIdentity`; pre-v22 saves render degraded MF reveal until the fight
...
## <a id="ISSUE-178"></a> ISSUE-178: 6 builds in the gen*.json mirror omit `nature`; the CSV source has natures for all 17,398 rows (mirror drift)
...
## <a id="ISSUE-179"></a> ISSUE-179: Engine-only `loadGameData` parse is ~308 ms (isolated from JSDOM), >1.5× the 200 ms boot target and scales with every new data table
...
## <a id="ISSUE-180"></a> ISSUE-180: `logMsg` runs an O(903-keys) `Object.keys(tooltipDict)` scan on every log line — 0.32 ms median, 13.9 ms max per call
...
## <a id="ISSUE-181"></a> ISSUE-181: 14 species have their ENTIRE standard-tier build pool tagged illegal — designed sets are dropped, foe falls back to randbats/Tackle
...
## <a id="ISSUE-182"></a> ISSUE-182: Pre-v15 post-HoF saves are forced back through the Mystery Figure climax — postHofMysteryClimaxDone migration shadowed by default boolean
...
## <a id="ISSUE-183"></a> ISSUE-183: 10 of 12 `.modal` overlays lack `role="dialog"` / `aria-modal` / labelledby
...
## <a id="ISSUE-184"></a> ISSUE-184: Only `modal-summary` handles Escape — settings, abandon, alert, confirm modals trap keyboard users
...
## <a id="ISSUE-185"></a> ISSUE-185: Modals restore focus on close but never move focus INTO the dialog on open
...
## <a id="ISSUE-186"></a> ISSUE-186: Precondition moves unimplemented — deal damage that should fail (Dream Eater, Thunderclap, Synchronoise)
...
## <a id="ISSUE-187"></a> ISSUE-187: Burn halving & Ice Scales key off `move.cat`, not `_effectiveCat` — wrong for Photon Geyser / Shell Side Arm
...
## <a id="ISSUE-188"></a> ISSUE-188: All damage modifiers collapsed into one multiply + single floor — Showdown floors per modifier (multi-HP drift)
...
## <a id="ISSUE-189"></a> ISSUE-189: In-game help text still says "PC Storage (cap 10)" — actual PC_BOX_CAP is 30
...
## <a id="ISSUE-190"></a> ISSUE-190: Pokémon Center storage rows are mouse-only clickable divs (no keyboard access)
...
## <a id="ISSUE-191"></a> ISSUE-191: 2-5 multi-hit distribution is 33/33/17/17, not the modern 35/35/15/15
...
## <a id="ISSUE-192"></a> ISSUE-192: Multi-hit moves reuse one damage roll & one crit check for every hit (no per-hit independence)
...
## <a id="ISSUE-193"></a> ISSUE-193: OHKO moves use the generic accuracy gate — affected by evasion/accuracy stages, Compound Eyes, Gravity; no higher-level auto-fail
...
## <a id="ISSUE-194"></a> ISSUE-194: "Vitamin" names three distinct systems — IV items, casino prize, EV voucher
...
## <a id="ISSUE-195"></a> ISSUE-195: Nurse Joy first-Center tutorial says PC has "ten slots" but PC_BOX_CAP is 30
...
## <a id="ISSUE-196"></a> ISSUE-196: End-of-turn residuals always resolve player-active-first, not in Speed order
...
## <a id="ISSUE-197"></a> ISSUE-197: Turn-loop median **23–38 ms / p95 35–53 ms / max 46–58 ms** in jsdom — production with `settings.animations=true` adds bounded `sleep()` delays on top, but the jsdom number IS the production floor when animations are off
...
## <a id="ISSUE-198"></a> ISSUE-198: 29 of 31 mart/dept catalog items (potion, superPotion, X items, orbs, etc.) have no entry in `data/items.json`
...
## <a id="ISSUE-199"></a> ISSUE-199: Mart/Dept consumables (30 ids: potion, xAttack, sunOrb, evResetCharm…) are a self-contained namespace, NOT entries in items.json
...
## <a id="ISSUE-200"></a> ISSUE-200: `_storyBattleEntryBusy` can latch true on a cold-open / beat-scene continuation throw → soft-locks "Enter Gym / Continue Route"
...
## <a id="ISSUE-201"></a> ISSUE-201: City-0 starter pick is drawn from a pure-G4 (weakest tier) pool
...
## <a id="ISSUE-202"></a> ISSUE-202: Mystery swap picker mislabels BST grade as "Power tier (1-4)"
...
## <a id="ISSUE-203"></a> ISSUE-203: README calls catch / PC / Underground / Safari / boss-arc "upcoming"; all are shipped
...
## <a id="ISSUE-204"></a> ISSUE-204: Catch screen result/throw text has no aria-live; outcomes silent to screen readers
...
## <a id="ISSUE-205"></a> ISSUE-205: City-hub Mystery Figure NPC sprite is hard-coded to Cyrus, ignoring `sm.mysteryIdentity` rotation
...
## <a id="ISSUE-206"></a> ISSUE-206: Pokémon Center copy promises a "Heal" that no longer exists (full-heal is automatic)
...
## <a id="ISSUE-207"></a> ISSUE-207: ISSUE-038 is marked fixed but `No Item` is still absent from items.json and 11 build slots still reference it
...
## <a id="ISSUE-208"></a> ISSUE-208: `_trainerPoolCache` is an unbounded Map (keyed on type+gens) with no eviction — Fight Club draft / story-pool variety will grow it without limit
...
## <a id="ISSUE-209"></a> ISSUE-209: Safari unlock spec'd "after badge 3 OR City3" but code (and REDESIGN) fix it firmly at City4
...
## <a id="ISSUE-210"></a> ISSUE-210: safari-zone integration test gives false confidence — asserts stale hard-coded weights and matches "1.25" anywhere in the spec doc
...
## <a id="ISSUE-211"></a> ISSUE-211: STORY_MODE_FLOW pins SAVE_VER at 15/17; shipped SAVE_VER is 21
...
## <a id="ISSUE-212"></a> ISSUE-212: SAVE_VER stays at 19 despite an implicit v20 schema cleanup that runs on every load
...
## <a id="ISSUE-213"></a> ISSUE-213: SAVE_VER=23 but migration chain stops at PreV22 — no migrateStoryPreV23 step (v23 added wanderByEventIdx, back-filled unconditionally)
...
## <a id="ISSUE-214"></a> ISSUE-214: Pending Healing Wish / Lunar Dance flags bleed into next battle and auto-heal its lead
...
## <a id="ISSUE-215"></a> ISSUE-215: sellItem trusts caller-supplied sellPrice instead of re-deriving from the catalog
...
## <a id="ISSUE-216"></a> ISSUE-216: 6 silent `catch (e) {}` blocks in online-pvp.js swallow all errors without logging
...
## <a id="ISSUE-217"></a> ISSUE-217: Display names accepted up to 24 chars with zero sanitization — flow into innerText today, but one careless innerHTML downstream becomes XSS
...
## <a id="ISSUE-218"></a> ISSUE-218: settings.animations defaults to true and is never seeded from prefers-reduced-motion
...
## <a id="ISSUE-219"></a> ISSUE-219: Snow Warning maps to Hail (gen-8) instead of gen-9 Snow
...
## <a id="ISSUE-220"></a> ISSUE-220: Anomaly seeds fire via low-z `showGameAlert` on the same tick as the row's flow — can paint behind/over other overlays
...
## <a id="ISSUE-221"></a> ISSUE-221: showGameConfirm overwrites a pending _gameConfirmResolve, orphaning the first awaiter
...
## <a id="ISSUE-222"></a> ISSUE-222: anime.js move-FX engine ignores prefers-reduced-motion — heaviest motion bypasses the CSS catch-all
...
## <a id="ISSUE-223"></a> ISSUE-223: Victory overlay is a plain `<div>` — no dialog role, no focus on Continue, ESC ignored
...
## <a id="ISSUE-224"></a> ISSUE-224: Fresh run starts with 0 Poké Balls; skipping the optional City-0 Mart silently no-ops the catch tutorial
...
## <a id="ISSUE-225"></a> ISSUE-225: Achievements `caged_god` / `r_caged_god` are permanently unobtainable (dead arc)
...
## <a id="ISSUE-226"></a> ISSUE-226: Caged God achievements (caged_god, r_caged_god) are permanently unearnable after v24 arc cut
...
## <a id="ISSUE-227"></a> ISSUE-227: STORY_MODE_FLOW says timeline is "68 rows"; STORY_EVENTS_RAW actually has 67 rows
...
## <a id="ISSUE-228"></a> ISSUE-228: Story spine is a hardcoded linear array; narrative layer is data-driven but no structural side-story/random-pool slots
...
## <a id="ISSUE-229"></a> ISSUE-229: Recurring facility-flavor pool covers 8 services; Safari / Stone Sage / Stone Shop / Dept Store have none
...
## <a id="ISSUE-230"></a> ISSUE-230: De-scoped features (Black Market, Illegal Dealer, Trader, Wager, full Itinerary) still presented as active spec — precise doc-edit list
...
## <a id="ISSUE-231"></a> ISSUE-231: Surviving canonical specs + code link to docs deleted in the cleanup (dangling references)
...
## <a id="ISSUE-232"></a> ISSUE-232: Story tone variants recolor nameplate text but not its yellow background — fails WCAG AA contrast
...
## <a id="ISSUE-233"></a> ISSUE-233: Story dialogue/NPC quote text is not in a live region — narration is silent to screen readers
...
## <a id="ISSUE-234"></a> ISSUE-234: Verbatim gold-HUD markup re-inlined 11× — the "re-inlined block 25 times" anti-pattern CLAUDE.md warns about
...
## <a id="ISSUE-235"></a> ISSUE-235: Help screen still advertises the cut Caged God / Subject Zero / Master-Ball quest
...
## <a id="ISSUE-236"></a> ISSUE-236: STORY_NARRATIVE_VARIANTS.md presents a cut 8-variant design as "canonical" (future-prompt-rebuild trap)
...
## <a id="ISSUE-237"></a> ISSUE-237: Cosmetic-skin roll references bare `sm` AND bare `storyRngNext` — double scope leak, never seeded
...
## <a id="ISSUE-238"></a> ISSUE-238: 351 it.todo() stubs across 3 move-category test files — cluster enumeration
...
## <a id="ISSUE-239"></a> ISSUE-239: 56 Gym Leaders have no entry in `TRAINER_QUOTES_BY_NAME` — fall through to generic 6-line `Gym Leader` pool
...
## <a id="ISSUE-240"></a> ISSUE-240: Trick / Switcheroo swap is one-directional — the user's item is destroyed
...
## <a id="ISSUE-241"></a> ISSUE-241: Tooltip-only data (type chart, move info, status terms) is `onmousemove`-gated — keyboard & touch get nothing
...
## <a id="ISSUE-242"></a> ISSUE-242: Long story replay shows steady, non-plateauing heap growth (~22–67 KB/turn, R²=0.99 over 250 battles) driven by per-turn / per-distinct-foe DOM scaffolding that GC + reset() do not reclaim
...
## <a id="ISSUE-243"></a> ISSUE-243: Long story replay shows linear (non-quadratic) heap + DOM-node growth that does not plateau — ~0.275 MB and ~52 sprite-container nodes retained per battle
...
## <a id="ISSUE-244"></a> ISSUE-244: STORY_3TRACK_IMPL_PLAN reads as a forward plan but is mostly SHIPPED — only PR-1 marked done
...
## <a id="ISSUE-245"></a> ISSUE-245: EVOLUTION_FLOW_REBUILD.md header says "Status: Plan — review before implementation" but the system fully shipped
...
## <a id="ISSUE-246"></a> ISSUE-246: Modals have aria-modal + Escape but no Tab focus trap — keyboard focus can leave the dialog
...
## <a id="ISSUE-247"></a> ISSUE-247: Heal phase (+25% maxHp) can push a raid boss back ABOVE the HP threshold the player just crossed
...
## <a id="ISSUE-248"></a> ISSUE-248: Dual story-vs-Frontier path in `_applyStoryBuildPowerTier` is mutually exclusive and safe — legacy branch is NOT removable (in-scope MF needs it)
...
## <a id="ISSUE-249"></a> ISSUE-249: Lead→city mapping duplicated (`_BOSS_LEAD_CITIES` const vs inline `_leadCity` literal)
...
## <a id="ISSUE-250"></a> ISSUE-250: Caged God uses three names for one entity (Specimen 0001 / Subject Zero / Subject 0001) without a stated rule
...
## <a id="ISSUE-251"></a> ISSUE-251: _bossArcRenderSection rebuilt in full inside every _renderCrucible re-render (adds ~6ms of the 30ms)
...
## <a id="ISSUE-252"></a> ISSUE-252: Non-hub Caged God render path is effectively dead post-HoF (player can never be at City 2/5/8)
...
## <a id="ISSUE-253"></a> ISSUE-253: Entire Caged God boss-arc subsystem is dead code after v24 removal
...
## <a id="ISSUE-254"></a> ISSUE-254: Solo-raid HP is 6.5× base, not the documented (maxParty-1)=5× — stat-mult and HP-scale compound on HP
...
## <a id="ISSUE-255"></a> ISSUE-255: CONFIRMED CLEAN — PC overflow at party-cap + 30/30 shows explicit message; sell/release path exists
...
## <a id="ISSUE-256"></a> ISSUE-256: Eggs occupy a party slot against the catch/withdraw cap but foe size matches only non-egg fighters — eggs silently shrink your catchable roster AND your opponent
...
## <a id="ISSUE-257"></a> ISSUE-257: Regular wild encounter with zero balls shows greyed buttons but no "out of balls" message
...
## <a id="ISSUE-258"></a> ISSUE-258: Variant Champion / rival dialogue narratively routes player to the dead broker + cage
...
## <a id="ISSUE-259"></a> ISSUE-259: "Free" badge wording inconsistent — "1st Free" vs "Free" vs "Claimed" vs "Locked"
...
## <a id="ISSUE-260"></a> ISSUE-260: 9 Gen-2-legacy "isBerry" items are dead data — no engine handler and never referenced by any build
...
## <a id="ISSUE-261"></a> ISSUE-261: Dead CSS selector #story-pc-tab-journal-btn — no such tab button exists in the Poké Center
...
## <a id="ISSUE-262"></a> ISSUE-262: `_pendingProfRoll` (singular) only ever assigned null — dead variable shadowing live `_pendingProfRolls`
...
## <a id="ISSUE-263"></a> ISSUE-263: Inert `_permBoostsRead`/`_permBoostTotal` stubs (+ window export) have zero callers — fully dead
...
## <a id="ISSUE-264"></a> ISSUE-264: `_pickCityQuoteLine` deliberately uses bare `Math.random` — drift across save reloads
...
## <a id="ISSUE-265"></a> ISSUE-265: Variant roll + Mystery identity use bare `Math.random()` at run construction — non-deterministic across seeded replays
...
## <a id="ISSUE-266"></a> ISSUE-266: League-road narrative "clumping" — 6 story beats fire back-to-back before the Champion (the audit §4 flow bug, still unfixed in the live path)
...
## <a id="ISSUE-267"></a> ISSUE-267: Sprite preloading is unbounded — each `getSprite()` call adds a `new Image()` to a global cache with no eviction; a long story run can preload 1000+ images
...
## <a id="ISSUE-268"></a> ISSUE-268: Casino debits floor gold at Math.max(0, gold-bet), which would silently mask a future bet-validation regression
...
## <a id="ISSUE-269"></a> ISSUE-269: Crucible "Pokémon Center" facility re-renders the Caged God section a second time (below the Underground sell list)
...
## <a id="ISSUE-270"></a> ISSUE-270: Grade badge prefix differs between prof pick cards (G#) and swap slots (T#)
...
## <a id="ISSUE-271"></a> ISSUE-271: "Reveal lands inside first ~10 minutes" comment is wrong — first villain beat is post-Gym-2 (road2)
...
## <a id="ISSUE-272"></a> ISSUE-272: Safari curve key [3] ("first unlock") is dead code — Safari actually unlocks at 4 badges, so first visit uses the harsher [4] curve
...
## <a id="ISSUE-273"></a> ISSUE-273: Safari grade weights are a per-badge curve in code, but STORY_MODE_FLOW.md §4 still specs the old flat g1:3/g2:22/g3:50/g4:25
...
## <a id="ISSUE-274"></a> ISSUE-274: Safari grade weights are now badge-keyed — spec/CODEBASE_MAP still cite the old static g1:3/g2:22/g3:50/g4:25
...
## <a id="ISSUE-275"></a> ISSUE-275: Safari grade weights diverge from the canonical spec (`g1:3/g2:22/g3:50/g4:25`) — code is a badge-staged curve; spec is stale
...
## <a id="ISSUE-276"></a> ISSUE-276: CONFIRMED CLEAN — catch tutorial fires exactly once; mid-tutorial reload cannot refire or lock
...
## <a id="ISSUE-277"></a> ISSUE-277: Post-HoF orientation tip frames the Mystery Figure as un-fought, but the row-67 climax already unmasked it
...
## <a id="ISSUE-278"></a> ISSUE-278: CORRECTION to prior audit: storyline variant is rolled randomly every run, NOT forced to 'classic'
...
## <a id="ISSUE-279"></a> ISSUE-279: faintPhase counts the active foe as "fainted" mid-tick if it is at 0 HP before the swap
...
## <a id="ISSUE-280"></a> ISSUE-280: bossMechanicsTurnTick per-turn cost is ~1.5us (foeParty.filter is NOT wasteful); only _showBossBanner DOM is non-trivial and fires ~5x/battle
...
## <a id="ISSUE-281"></a> ISSUE-281: Redundant tier branches in `_storyBuildTierForEvent` (dead duplicate conditions)
...
## <a id="ISSUE-282"></a> ISSUE-282: `_storyBuildTierForEvent` Basic-Trainer branch has a dead `b>=5` arm — route fodder jumps T2→T4 at post-game with no T3 step (half-applied curve edit)
...
## <a id="ISSUE-283"></a> ISSUE-283: "wild < gym staff" ladder is violated — Basic Trainer and Gym Trainer 1 share the SAME tier at every badge count
...
## <a id="ISSUE-284"></a> ISSUE-284: Basic Trainer build-tier ladder collapses at Stage 2 — same tier as Gym Trainers despite the "one tier below" comment
...
## <a id="ISSUE-285"></a> ISSUE-285: `_storyEnemyMechKeys` doesn't guard against missing `sm.settings` — could throw on a malformed save
...
## <a id="ISSUE-286"></a> ISSUE-286: Mystery Figure sprite is now `Red` (the_first); the `'Cyrus'` fallback at enterBattleEvent is dead
...
## <a id="ISSUE-287"></a> ISSUE-287: Stale comment on `_storyGrantTrackEndReward` — claims scene-queue piggy-back that is structurally impossible
...
## <a id="ISSUE-288"></a> ISSUE-288: Extra-track raid EXP-Share reward + boss BOSS_MECHANICS are partly data-only — engine wiring deferred (mechanics are no-ops that only record)
...
## <a id="ISSUE-289"></a> ISSUE-289: CONFIRMED CLEAN — party-cap curve = min(6, 2+badges) with no off-by-one; foe sizing matches
...
## <a id="ISSUE-290"></a> ISSUE-290: Legacy storyline picker is dead UI — hidden DOM + uncalled renderer + unreachable card handlers, superseded by sm.tracks
...
## <a id="ISSUE-291"></a> ISSUE-291: `_validateTrainerData` logs a success `console.log` on every boot (ungated)
...
## <a id="ISSUE-292"></a> ISSUE-292: Variant rival quote pools are uneven — several phases have a single line; many phases absent
...
## <a id="ISSUE-293"></a> ISSUE-293: Wild grade keyed on CITY (STORY_WILD_GRADE_BY_CITY), not badges — spec §3/§15f say _WILD_GRADE_CURVE_BY_BADGES (which does not exist)
...
## <a id="ISSUE-294"></a> ISSUE-294: CONFIRMED CLEAN — mechanics unlock gate has no leak on any player or enemy path
...
## <a id="ISSUE-295"></a> ISSUE-295: items.json defines 93 mega stones but the engine recognizes only 51 — 45 non-canonical stones are inert data
...
## <a id="ISSUE-296"></a> ISSUE-296: Anomaly seeds are keyed by row ID but several land on mismatched event types vs their prose
...
## <a id="ISSUE-297"></a> ISSUE-297: Latent state-bleed: artifact battle-flags reset is behind an empty-artifacts early-return (same init-inside-guard shape as the fixed boss-bleed)
...
## <a id="ISSUE-298"></a> ISSUE-298: `applyFoeDifficultyScaling`'s "additive league boost" comment narrates a fix that was never implemented
...
## <a id="ISSUE-299"></a> ISSUE-299: Spec §8 says league boost stacks multiplicatively with difficulty; code now stacks additively (the cliff was fixed)
...
## <a id="ISSUE-300"></a> ISSUE-300: Spec §8 says league boost stacks MULTIPLICATIVELY with difficulty; code now stacks ADDITIVELY
...
## <a id="ISSUE-301"></a> ISSUE-301: Magma/Aqua bosses flash the same telegraph banner twice in the first two turns
...
## <a id="ISSUE-302"></a> ISSUE-302: `BOSS_MECHANICS` stub object is dead (never called); its "engine wiring is a no-op" comment is now false
...
## <a id="ISSUE-303"></a> ISSUE-303: `BOSS_MECHANICS` registry (`~42172`) is dead — pushes to `battle._mechanics`, which is never read
...
## <a id="ISSUE-304"></a> ISSUE-304: Dead `build.tired` fatigue field still written/backfilled at 5 sites, read in zero gameplay paths
...
## <a id="ISSUE-305"></a> ISSUE-305: Extra-raid stat scaling compounds `_storyStatMult` × `_bossStatMult` × `_bossHpScale`; the doc comment omits `_storyStatMult`
...
## <a id="ISSUE-306"></a> ISSUE-306: buyItem cheap-consumable path is unguarded but synchronous; only the confirm-gated branch can interleave
...
## <a id="ISSUE-307"></a> ISSUE-307: Achievements caged_god / r_caged_god are permanently unobtainable
...
## <a id="ISSUE-308"></a> ISSUE-308: Roulette doc comment promises a color-row payout the code never pays
...
## <a id="ISSUE-309"></a> ISSUE-309: Integration test asserts stale "PC cap of 10" and never checks the real PC_BOX_CAP=30
...
## <a id="ISSUE-310"></a> ISSUE-310: `sm.catchUnlocked` is written by migration + newStoryRun but never read anywhere
...
## <a id="ISSUE-311"></a> ISSUE-311: sm.catchUnlocked written by defaults + v15 migration + newStoryRun but read nowhere (live gate is sm.catchTutorialDone)
...
## <a id="ISSUE-312"></a> ISSUE-312: CHAMPION_VICTORY_LINES['Hau'] is dead — Hau is an Elite Trainer, never a Champion
...
## <a id="ISSUE-313"></a> ISSUE-313: CHANGELOG 2026-05-21 entry claims Cable Link is ungated but the implementation in v18 explicitly gates it
...
## <a id="ISSUE-314"></a> ISSUE-314: `createRoom` hardcodes Postgres SQLSTATE `23505` for unique-violation detection — defensive `.includes('duplicate')` fallback exists but the magic number is undocumented
...
## <a id="ISSUE-315"></a> ISSUE-315: Crucible rematch pickers use bare Math.random — breaks the seeded-replay contract for post-game
...
## <a id="ISSUE-316"></a> ISSUE-316: ELITE_VICTORY_LINES['Molayne'] is dead — Molayne is an Elite Trainer, not an E1–E4 boss
...
## <a id="ISSUE-317"></a> ISSUE-317: Leech Seed drain is processed AFTER burn/poison/toxic damage (canon order is before)
...
## <a id="ISSUE-318"></a> ISSUE-318: Relic Annex intro uses plain-text `_storyShowOneTimeTip`; every other facility uses a sprite-backed scene
...
## <a id="ISSUE-319"></a> ISSUE-319: enterArtifactShop and enterShop lack the _storyTryBeginInteraction guard used by other facility entries
...
## <a id="ISSUE-320"></a> ISSUE-320: City0 welcome tip says the Underground "buys … never your starter" but starters are sellable
...
## <a id="ISSUE-321"></a> ISSUE-321: Crucible sub-sections improve wayfinding but the orientation tip + "Mystery vs Caged God" disambiguation still lean on long alert text
...
## <a id="ISSUE-322"></a> ISSUE-322: Poké Center chip sits in "Heal & Team" section with a "Free" badge but performs no heal interaction
...
## <a id="ISSUE-323"></a> ISSUE-323: Story facility regions use weak lowercase aria-labels ("story pokemoncenter", "story link")
...
## <a id="ISSUE-324"></a> ISSUE-324: `enterProfessor` duplicates gimmick-gate logic inline instead of using `_withStoryPlayerGimmickGate`
...
## <a id="ISSUE-325"></a> ISSUE-325: `enterProfessor` reuses `_pendingProfChoices` across city visits at the same cityIdx — stale picks may persist past spec'd one-shot pool
...
## <a id="ISSUE-326"></a> ISSUE-326: Professor flavor quote uses bare Math.random(), breaking seeded replay determinism
...
## <a id="ISSUE-327"></a> ISSUE-327: Empty-choices Professor path shows status but renders no body buttons
...
## <a id="ISSUE-328"></a> ISSUE-328: Exp Share Voucher item (3TRACK_IMPL_PLAN PR-5) never shipped; `sm.inventory.expShareVoucher` is dead init
...
## <a id="ISSUE-329"></a> ISSUE-329: `expShareVoucher:0` inventory field is dead — no reader, no writer; extra-raid reward grants vitamins instead
...
## <a id="ISSUE-330"></a> ISSUE-330: Service-availability timeline reference (Task 1 deliverable) — first-appearance / reappear / unlock map
...
## <a id="ISSUE-331"></a> ISSUE-331: Crucible-reachable Frontier surrender uses raw window.confirm — drops fullscreen, breaks modal convention
...
## <a id="ISSUE-332"></a> ISSUE-332: Gauntlet score readout is a plain div with no live region — score changes are silent to SR
...
## <a id="ISSUE-333"></a> ISSUE-333: Paralysis tooltip says "Speed quartered" but engine halves speed (0.5) — stale Gen 1-6 text vs Gen 7+ behavior
...
## <a id="ISSUE-334"></a> ISSUE-334: Rival phase enum skips 1 (EARLY rival returns phase 2), leaving a dead phase-1 dialogue pool
...
## <a id="ISSUE-335"></a> ISSUE-335: Sprite preload cache `_preloadedImages` is still an unbounded Object with no eviction — every distinct (name, shiny, back) pins an Image() for the session
...
## <a id="ISSUE-336"></a> ISSUE-336: Two disjoint "beat" systems — row-id `STORY_BEATS` (cold-opens) vs sceneKey `*_STORY_BEATS` (3-track)
...
## <a id="ISSUE-337"></a> ISSUE-337: Featured Mega/Ultra stones (bought one-per-city at 5x/3x) are sellable from the bag at half list price
...
## <a id="ISSUE-338"></a> ISSUE-338: Grass Whistle never puts the target to sleep
...
## <a id="ISSUE-339"></a> ISSUE-339: CONFIRMED FIXED — GYM_CITY_LEADER_EVENT is now derived from STORY_EVENTS_RAW at boot (prior audit 1.3)
...
## <a id="ISSUE-340"></a> ISSUE-340: `isPokeball` flag set on 28 items but never read by the engine — dead metadata
...
## <a id="ISSUE-341"></a> ISSUE-341: 9 legacy gen2 berries (Bitter, Burnt, Gold, Ice, Mint, Miracle, Mystery, PRZ Cure, PSN Cure) have no engine handler
...
## <a id="ISSUE-342"></a> ISSUE-342: `delete sm.casinoCoins` cleanup runs on every load, not gated by `_loadedVer`
...
## <a id="ISSUE-343"></a> ISSUE-343: CONFIRMED CLEAN — full migrate chain v8→v21 round-trips pre-v15 saves without crash or party/PC/badge loss
...
## <a id="ISSUE-344"></a> ISSUE-344: Engine cold-boot is 2.88 s in jsdom — within the harness target (5 s) but **14× the mandate's 200 ms target**
...
## <a id="ISSUE-345"></a> ISSUE-345: Test-harness docs promise window.SAVE_VER / window.sm / window.newStoryRun but only StoryMode + __storyLoad are exposed
...
## <a id="ISSUE-346"></a> ISSUE-346: `console.log` cluster in battle.html — debug noise in shipped code
...
## <a id="ISSUE-347"></a> ISSUE-347: Cold boot is ~3.0 s in jsdom (5-process median 3009 ms) — within the harness's relaxed 5 s self-target but 15× the mandate's 200 ms; a target-mismatch to resolve, not a regression
...
## <a id="ISSUE-348"></a> ISSUE-348: Grade badge prefix differs — `G{tier}` on draft cards vs `T{grade}` on swap/daycare slots
...
## <a id="ISSUE-349"></a> ISSUE-349: Button label patterns inconsistent — Professor uses verb+noun, all others noun-only; Evolution facility has 3 names
...
## <a id="ISSUE-350"></a> ISSUE-350: Empty-state copy varies across facilities for the same "no party member" condition
...
## <a id="ISSUE-351"></a> ISSUE-351: 7 build abilities (Telepathy/Mountaineer/Friend Guard/Healer/Pickup/Rebound/Symbiosis) are silent no-ops the engine never implements
...
## <a id="ISSUE-352"></a> ISSUE-352: `makeBuild` is flat across power tiers (T1-T4 spread <0.04 ms median) — confirms no per-tier pathology; baseline 0.045 ms median
...
## <a id="ISSUE-353"></a> ISSUE-353: v15 stable-ID migration only iterates `sm.team`, not `sm.pcBox` (harmless today but inconsistent with v17)
...
## <a id="ISSUE-354"></a> ISSUE-354: Pre-v15 saves get 0 Poké Balls instead of the intended 5 — migrateStoryPreV15 balls default is shadowed by the default sm object
...
## <a id="ISSUE-355"></a> ISSUE-355: Catch-tutorial migration hard-codes intro-rival index (>1) instead of deriving it
...
## <a id="ISSUE-356"></a> ISSUE-356: catchTutorialDone migration hardcodes eventIndex>1 instead of deriving the intro-rival row
...
## <a id="ISSUE-357"></a> ISSUE-357: catchTutorialDone migration hard-codes `eventIndex > 1` instead of deriving the intro-rival array index
...
## <a id="ISSUE-358"></a> ISSUE-358: Online Host/Join form labels are not programmatically associated with their inputs
...
## <a id="ISSUE-359"></a> ISSUE-359: STORY_MODE_FLOW §14d describes Mystery Figure's 7-identity flow + Caged God repurpose; code has only the_first
...
## <a id="ISSUE-360"></a> ISSUE-360: CONFIRMED FIXED — Mystery Figure is now a rotating 10-identity cast (prior audit: hardcoded Cyrus)
...
## <a id="ISSUE-361"></a> ISSUE-361: `mysteryBias` per-variant config is orphaned — seeds weights for retired MF identities, never read
...
## <a id="ISSUE-362"></a> ISSUE-362: Mystery Figure rotating-cast scaffolding is now vestigial — collapsed to a single hard-locked identity "the_first"
...
## <a id="ISSUE-363"></a> ISSUE-363: Fresh runs start with 0 Poké Balls; spec §1/§10 say 5 (only migrated saves get 5)
...
## <a id="ISSUE-364"></a> ISSUE-364: A cluster of form controls lack accessible names (online host/join, casino bet, gauntlet opt-in)
...
## <a id="ISSUE-365"></a> ISSUE-365: `parseCSV` of `data/builds.csv` (2.6 MB, 17,397 rows) blocks the main thread for **180 ms median** during boot
...
## <a id="ISSUE-366"></a> ISSUE-366: `parseMoveEffects` per-move latency varies by ~315× between fastest and slowest moves; outliers are 25–250× the median
...
## <a id="ISSUE-367"></a> ISSUE-367: `parseMoveEffects` per-move latency varies ~257× (median 0.012 ms, slowest 3.19 ms) — multi-stat-boost / "dance" moves are the outliers
...
## <a id="ISSUE-368"></a> ISSUE-368: `parseMoveEffects` re-allocates 19 constant `Set` literals on every call and pays a 2–14 ms one-time JIT cost on first touch of each branch (warm cost is fine at ~0.01 ms)
...
## <a id="ISSUE-369"></a> ISSUE-369: parseMoveEffects per-move spread is 130x (stat-stage moves ~1.3ms vs 0.01ms median) — benign, multiple changeStage calls
...
## <a id="ISSUE-370"></a> ISSUE-370: Burn applied as final-damage multiplier (`modifier *= 0.5`) instead of halving the attack stat pre-floor
...
## <a id="ISSUE-371"></a> ISSUE-371: Stat-change status moves (Decorate, Coaching, Baby-Doll Eyes, Calm Mind) are 50–100× slower than damage moves — `changeStage` calls `logMsg` 1–3× per stat tick, each hitting the 903-key tooltipDict scan
...
## <a id="ISSUE-372"></a> ISSUE-372: Hand-quantification of parseMoveEffects Set-literal churn — 18 `new Set([...])` per call, 13 μs of pure allocation per call (≈70% of warm 18 μs median)
...
## <a id="ISSUE-373"></a> ISSUE-373: Several status moves have no observable effect in the battle engine
...
## <a id="ISSUE-374"></a> ISSUE-374: 129 `@keyframes` definitions but only 5 `prefers-reduced-motion` overrides — sprite-in, dialog-in, master-pulse, badge-pulse all unguarded
...
## <a id="ISSUE-375"></a> ISSUE-375: proceedToNextBattle guards on total team length, but the launch path guards on non-egg fighter count — an all-egg party advances eventIndex then bounces
...
## <a id="ISSUE-376"></a> ISSUE-376: proceedToNextBattle "no Pokémon" guard counts eggs (team.length) while the fight launch counts only fighters — egg-only party advances then bounces
...
## <a id="ISSUE-377"></a> ISSUE-377: Mystery-mode Accept has no double-submit guard (re-renders swap picker on repeat clicks)
...
## <a id="ISSUE-378"></a> ISSUE-378: Room codes use `Math.random()` over 32 chars × 6 positions — 30 bits, birthday collision at ~30K concurrent rooms, retry-on-23505 saves correctness but not enumeration cost
...
## <a id="ISSUE-379"></a> ISSUE-379: README calls shipped catch / PC / Underground / Safari / boss-arc systems "upcoming"
...
## <a id="ISSUE-380"></a> ISSUE-380: Nature Rater availability is gappy (C0, C3, C5–C9) — absent C1/C2/C4 with no unlock rationale
...
## <a id="ISSUE-381"></a> ISSUE-381: Rival-gate tip labelled "Heal …" deep-links to the Poké Center, which performs no heal
...
## <a id="ISSUE-382"></a> ISSUE-382: Dead `'Cyrus'` Mystery-Figure sprite fallbacks remain after the identity was collapsed to a single value ('the_first' / Red)
...
## <a id="ISSUE-383"></a> ISSUE-383: Party count chip shows "(N/6)" regardless of the actual badge-driven cap
...
## <a id="ISSUE-384"></a> ISSUE-384: CONFIRMED FIXED — RIVAL_ATTACK_TYPE_DECAY is now 10 (prior audit 1.2 had ÷30 too-aggressive)
...
## <a id="ISSUE-385"></a> ISSUE-385: Mystery Figure intro pool fallback uses 2 lines but the identity's `intros` field has 4 — falls back silently if the identity object lacks `intros`
...
## <a id="ISSUE-386"></a> ISSUE-386: Mystery Figure climax boss has ZERO gimmicks if the player disabled all 4 mechanics at run start — the "force all on" ctx is dead-coded
...
## <a id="ISSUE-387"></a> ISSUE-387: `rollTrainerTeam` cold-call is **1.63 ms median (max 3.22 ms)**; well under the 50 ms target but worth recording as the deep-dive baseline before the upcoming difficulty-curve work
...
## <a id="ISSUE-388"></a> ISSUE-388: Safari flee-risk surfaced in color (orange #ff7043) and small 10px text only
...
## <a id="ISSUE-389"></a> ISSUE-389: Move-test generator strips apostrophes, and the engine silently runs unknown move names as a 187-dmg fallback
...
## <a id="ISSUE-390"></a> ISSUE-390: SAVE_VER=23 but migration chain stops at `_loadedVer < 22` — no migrateStoryPreV23, no boot shadow-validation
...
## <a id="ISSUE-391"></a> ISSUE-391: All 24 `#screen-*` containers are plain `<div>` — no `<main>` or `role="region"`
...
## <a id="ISSUE-392"></a> ISSUE-392: Several story-mode dev seeds use `Math.random` for build / sprite picks, breaking seeded replays when debug seeds are in play
...
## <a id="ISSUE-393"></a> ISSUE-393: End-of-turn residual logic is duplicated verbatim in forced-switch path and main loop — divergence risk
...
## <a id="ISSUE-394"></a> ISSUE-394: Player gimmick-unlock order shifts when a mechanic is disabled in settings — DMax unlocks at Gym 5 instead of Gym 6 if Mega is off
...
## <a id="ISSUE-395"></a> ISSUE-395: `shouldForceCityProfessor` uses dead `sm.team.length < 6` floor that's shadowed by outer `hasTeamRoom` gate
...
## <a id="ISSUE-396"></a> ISSUE-396: `shouldForceCityProfessor`'s `sm.team.length < 6` floor is dead-coded once the outer `hasTeamRoom` gate clamps to `_storyMaxPartySize()`
...
## <a id="ISSUE-397"></a> ISSUE-397: Stale comment claims rival secondary intro "Uses Math.random" — it now uses seeded _storySideRng
...
## <a id="ISSUE-398"></a> ISSUE-398: Mobile move-details "i" is a span[role=button] with no tabindex/keydown — keyboard cannot reach it
...
## <a id="ISSUE-399"></a> ISSUE-399: Victory overlay auto-dismisses after 6s regardless of how much narrative it stacks — the biggest story beats can vanish before they're read
...
## <a id="ISSUE-400"></a> ISSUE-400: Subject Zero stored to PC (party-full at cage) shows "Subject Zero" nickname but is never auto-fielded — easy to miss the capstone mon
...
## <a id="ISSUE-401"></a> ISSUE-401: Inconsistent auto-dismiss across scene types — victory 6s timeout vs beat scenes never auto-dismiss
...
## <a id="ISSUE-402"></a> ISSUE-402: Doc battle.html:LINE anchors are stale (23/43 drifted across the spec docs)
...
## <a id="ISSUE-403"></a> ISSUE-403: Mystery Figure identity is rolled at run start before sm.active/runSeed are live — not reproducible under fixed debug seeds
...
## <a id="ISSUE-404"></a> ISSUE-404: Row-67 `STORY_BEATS` still tags `'cagedGod'` + coldOpen `mystery67` — stale cut-arc residue in the live beat map
...
## <a id="ISSUE-405"></a> ISSUE-405: Internal action keys use `Pokemon` (no diacritic) while UI labels use `Pokémon`
...
## <a id="ISSUE-406"></a> ISSUE-406: Doc line anchors stale — 18/50 `battle.html:LINE` refs in design docs no longer resolve (clustered)
...
## <a id="ISSUE-407"></a> ISSUE-407: Doc `battle.html:LINE` anchors stale — 18/50 references drifted (cluster)
...
## <a id="ISSUE-408"></a> ISSUE-408: Doc `battle.html:LINE` anchors stale in surviving specs — 24/44 drifted (post-cleanup cluster; updates ISSUE-330)
...
## <a id="ISSUE-409"></a> ISSUE-409: Service-timeline pacing — City1 post-gym hub is a dead zone (no new "thing to do")
...
## <a id="ISSUE-410"></a> ISSUE-410: STORY_EVENTS_RAW has 67 array rows (incl. 2 'Hall of Fame' string matches) — mandate/spec cite "68 rows"
...
## <a id="ISSUE-411"></a> ISSUE-411: STORY_EVENTS_RAW resolves to 67 rows in harness vs 68 stated in spec/mandate
...
## <a id="ISSUE-412"></a> ISSUE-412: Timeline is 67 rows; STORY_MODE_FLOW.md (and this update's brief) still say "68 rows"
...
## <a id="ISSUE-413"></a> ISSUE-413: STORY_MODE_FLOW §4 still specs the flat Safari weights g1:3/g2:22/g3:50/g4:25; live code is a badge curve (_SAFARI_GRADE_CURVE_BY_BADGES)
...
## <a id="ISSUE-414"></a> ISSUE-414: `STORY_IV_TIER_RANGES` is dead — superseded by `STORY_IV_TIER_CENTER`, zero consumers
...
## <a id="ISSUE-415"></a> ISSUE-415: THREE story IV tables coexist — `STORY_IV_TIER_RANGES` is fully dead; `STORY_IV_TIER_CENTER` is Frontier-only; only `STORY_IV_CITY_*` is the live story curve
...
## <a id="ISSUE-416"></a> ISSUE-416: docs/STORY_MODE_AUDIT.md is stale — most of its flagged issues are now fixed (SAVE_VER 14→22)
...
## <a id="ISSUE-417"></a> ISSUE-417: 6+ stale `battle.html:LINE` refs in STORY_MODE_CATCH_INTEGRATION_RISK.md (avg drift ~9000 lines)
...
## <a id="ISSUE-418"></a> ISSUE-418: Doc `battle.html:LINE` anchors still stale (50 refs, 18 drifted) despite PR #140 "fix"
...
## <a id="ISSUE-419"></a> ISSUE-419: 9 of 10 `battle.html:LINE` refs in STORY_MODE_FLOW.md are stale (avg drift ~7000 lines)
...
## <a id="ISSUE-420"></a> ISSUE-420: STORY_MODE_FLOW.md PC cap of 10 contradicts shipped PC_BOX_CAP=30
...
## <a id="ISSUE-421"></a> ISSUE-421: Doc line anchors stale across 4 specs (still drifting post-v24; cluster)
...
## <a id="ISSUE-422"></a> ISSUE-422: 5+ stale `battle.html:LINE` refs in STORY_NARRATIVE_VARIANTS.md (variant-system anchors moved 3-5k lines)
...
## <a id="ISSUE-423"></a> ISSUE-423: Spec/mandate says timeline is "68 rows"; STORY_EVENTS_RAW has 67 (array idx 0–66), and rowId 68 is the intro Rival at array idx 1
...
## <a id="ISSUE-424"></a> ISSUE-424: Tutorial scenes are text-only walls of 60-120 words — no audio, no progressive reveal, no skip-to-end affordance
...
## <a id="ISSUE-425"></a> ISSUE-425: Catch ball buttons lack accessible name reading the success %; bare-icon img alt empty
...
## <a id="ISSUE-426"></a> ISSUE-426: Crucible & Catch headers use empty spacer spans instead of a back control; no escape from Crucible header
...
## <a id="ISSUE-427"></a> ISSUE-427: Pokédex counts strip updates live (seen/caught) but is not an aria-live region
...
## <a id="ISSUE-428"></a> ISSUE-428: Dead CSS selector for a #story-pc-tab-journal-btn that has no markup or handler
...
## <a id="ISSUE-429"></a> ISSUE-429: Pokémon Center tab buttons not exposed as a tablist (no role=tab/tabpanel)
...
## <a id="ISSUE-430"></a> ISSUE-430: Mobile touch targets in story shops top out at 42px — under the WCAG 44×44 baseline
...
## <a id="ISSUE-431"></a> ISSUE-431: Tutorial overlay's four-stage entrance animation has no reduced-motion fallback
...
## <a id="ISSUE-432"></a> ISSUE-432: Scope-leak audit (same class as fixed `sm` bug) — NEGATIVE: no other IIFE-internal symbol referenced bare from turn-loop scope
...
## <a id="ISSUE-433"></a> ISSUE-433: `storyAwareRng()` helper exists but the combat engine still calls bare `Math.random()` at ~250 sites
...
## <a id="ISSUE-434"></a> ISSUE-434: Master Ball purple pulse runs forever with no reduced-motion override; surrounding text/glow stays readable but the loop is hostile
...
## <a id="ISSUE-435"></a> ISSUE-435: CONFIRMED FIXED — Hard coin mult floored to 1.00 (prior audit 2.1); Challenge 1.10
...
## <a id="ISSUE-436"></a> ISSUE-436: Hard mode still earns less gold per fight than Normal (1.00 vs 1.30) despite facing 1.15x-stronger foes — residual difficulty/economy asymmetry
...
## <a id="ISSUE-437"></a> ISSUE-437: `storyRngNext` / per-engine-entry `rng` ternary accessor is **0.0003 ms (300 ns) per call** — no closure allocation penalty, accessor pattern is clean
...
## <a id="ISSUE-438"></a> ISSUE-438: Status indicator toggles role="status" + aria-label onto a <div> at content-mutation time
...
## <a id="ISSUE-439"></a> ISSUE-439: Upper Hand / Shell Trap don't enforce their precondition gate
...
## <a id="ISSUE-440"></a> ISSUE-440: `sm.wildSeenByEventIdx` and `sm.staticDrops` are lazily initialized instead of declared in the `sm` defaults block
...
## <a id="ISSUE-441"></a> ISSUE-441: VERIFIED OK — boss/raid reward double-grant across the two call sites is structurally prevented
...
## <a id="ISSUE-442"></a> ISSUE-442: `_bossWeatherLocked` / `_bossTerrainLocked` flags are set but never read — field "lock" is opening-state only
...
## <a id="ISSUE-443"></a> ISSUE-443: VERIFIED OK — extra-arc raid "laid to rest, no catch" lock is enforced structurally
...
## <a id="ISSUE-444"></a> ISSUE-444: `SAVE_VER = 23` but migration dispatch stops at `_loadedVer < 22` — no numbered v23 step
...
## <a id="ISSUE-445"></a> ISSUE-445: Three redundant RNG-routing mechanisms; confusion self-hit relies solely on the global Math.random patch while siblings use storyAwareRng()
