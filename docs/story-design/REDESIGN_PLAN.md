...
**Remaining — the feature tranche (needs SAVE_VER 20→21 + browser/harness verification):** B daycare C2/C4/C6 + relative hatch + animation, C Fight Club gauntlet (5-round 3v3 draft + counter-pick + IV reward), G expansion-pool content on the INTERRUPTS bus, K1 facility revisit quotes (Safari/Dept/Stone). **Your call:** E1 AI-competence axis. These are now de-risked: migration is tested (L1), the draft render is a11y-ready (I1), and the caches the new systems lean on are bounded (J1/J2).
