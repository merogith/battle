...
**✅ Shipped & tested this session (9 commits, all migration-free):** D3+D4 (foe-mult monotonic ramp 0.95→0.97→0.99→1.01→1.03→1.05→1.08→1.10 — un-flattens GL1≡GL2 + fills the GL4≡GL5 dead zone), D7 (doc corrected — wilds already low-grade), D6 (Colress price reconciled to 7,500G across 5 surfaces), H1 (League/Fan Club diacritic + dispatch), H2 (cries local-first + CDN fallback), H3 (paralysis tooltip), H4 (PC cap 30), F1 (rivalry journal → Collection), J1+J2 (both unbounded caches bounded), I1 (draft cards keyboard/SR-operable), I2 (`showScreen` focus delivery), **L1 (save-migration test made real — the prerequisite for any SAVE_VER bump)**.
**Closed by design / non-issue:** D5 (GL8 IV step is the *locked* climax; foe-mult already smoothed — no discrete-tier refactor), H5 (no Center "Heal" action exists; the between-fights heal copy is correct).
**✅ Features shipped & tested (12 commits total):**
- **B — daycare relative hatch (done):** eggs stamp `eggLaidAtCity` and hatch at pickup-city + 2 (replaces the rigid Gym-7 gate), with the "egg is stirring" foreshadow toast and a **SAVE_VER 20→21 + `migrateStoryPreV21`** stamping legacy eggs. *(Deferred sub-part: the C2/C4/C6 facility re-gate — current every-city-after-Gym1 availability is more generous and not broken; the hatch *animation* needs a browser.)*
- **C — Fight Club 5-round gauntlet + per-round reward (partial):** bracket extended 3→5 rounds (boss is the last fight), reward changed from flat +1-all to **+1 per round won, clamp +5** (a full sweep = +5, the intended catch-up). *(Deferred sub-part: the 3v3 draft + enemy counter-pick flow and the C9 pre-League instance — these rebuild the battle-launch path and are unsafe to ship without a browser smoke-test.)*

**Remaining (genuinely needs a browser pass or is optional):** the daycare hatch **animation** + C2/C4/C6 re-gate, the Fight Club **3v3 draft + counter-pick** + C9 instance, **G** expansion-pool content on the INTERRUPTS bus, **K1** facility revisit quotes (Safari/Dept/Stone — additive content). **Your call:** **E1** AI-competence axis.
