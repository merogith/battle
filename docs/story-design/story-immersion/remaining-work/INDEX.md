...
> **⚠ Read first — the Camp System is its own track.** The biggest "remaining" item
> (the navigable camps + bond/relation + minigames feature) is **NOT** captured here. It
> has a full, maintainer-approved design package at **[`../../camp/`](../../camp/)** with its
> own [`IMPLEMENTATION_ROADMAP.md`](../../camp/IMPLEMENTATION_ROADMAP.md) (PRs A–F) and a
> [2026-06-08 decisions addendum](../../camp/DECISIONS_2026-06-08.md). **TASK-01 below is
> SUPERSEDED by that package.** This folder now covers only the **Stream-4 story-immersion
> wiring** (TASK-02..05).
...
|---|---|---|---|---|
| **TASK-01** — Navigable Nodes & Camps | ⚠ **SUPERSEDED** → use [`../../camp/`](../../camp/) | — | — | — |
...
CAMP SYSTEM (../../camp/) ─────────── the BIG one, its own roadmap (PRs A–F). Note: H3-2's
                                     diegetic surface (TASK-02) is the surface the camp
                                     "diamonds" reuse, so TASK-02 first de-risks camp PR C.
```

**Recommended order:** TASK-02 → TASK-03 → TASK-04 → TASK-05 (the contained wins), and run
the **Camp System** as its own sign-off-gated track per its roadmap.
...
- `../../camp/` — the Camp System package (supersedes TASK-01).
- `../../STORY_FLOW_AUDIT.md` — §7a node-based flow, §8e NAV-2 (the one camp piece deferred to v2).

## Ground rules (from `CLAUDE.md`)

- No game-behavior change ships without sign-off (TASK-04, D3(b) in TASK-05, and all camp code).
