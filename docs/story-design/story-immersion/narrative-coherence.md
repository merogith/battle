...
| **G9** | **P2** | **`STORY_SCENES.*.body` still carries raw stage-directions.** Legacy `body` fields hold "Phase 3 at 25% … takes no damage from physical attacks for 2 turns" (e.g. `extra.cubone.raid`). The live `acts` render is clean, but any fallback path that reads `body` leaks mechanics into prose. | `STORY_SCENES` (`body` fields on battle scenes) | A latent "out of nowhere" leak: stage-directions are not narration. Low severity only because the `acts` path is the live one today. |
