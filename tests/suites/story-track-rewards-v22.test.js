// PR-5: track-end rewards (Master Ball, EXP-Share gift) + BOSS_CONFIGS registry.
// Run: node --test tests/suites/story-track-rewards-v22.test.js
//
// NOTE (EXP-Share rework): the extra-raid reward now grants the EXP-Share GIFT
// — EXP_SHARE_GIFT_UNITS (6) distributable level-ups parked in the wallet
// sm.inventory.expShareVoucher — instead of the old 6-random-vitamin bundle.
// _storyGrantTrackEndReward is now idempotent per sceneKey (sm._trackRewardGranted),
// so every reset below clears that flag to keep each case independent.
...
test('grantTrackEndReward grants Master Ball on any villain.X.boss', () => {
    ST.sm = Object.assign({}, ST.sm, { balls: { poke:0, great:0, ultra:0, master:0 }, _trackRewardGranted: {} });
...
test('grantTrackEndReward grants the EXP-Share gift (6 units to the wallet) on any extra.X.raid', () => {
    ST.sm = Object.assign({}, ST.sm, { inventory: {}, _trackRewardGranted: {} });
    const result = ST.grantTrackEndReward({ sceneKey: 'extra.cubone.raid' });
    assert.equal(result.kind, 'expShareGift');
...
test('grantTrackEndReward works for all 8 extra tracks', () => {
    ...
    for (const x of extras) {
        ...
        assert.ok(result && result.kind === 'expShareGift', `failed for extra ${x}`);
        assert.equal(ST.sm.inventory.expShareVoucher | 0, 6, `extra ${x} gift != 6 units`);
    }
});

test('grantTrackEndReward accumulates (Master Ball + EXP-Share gift stack)', () => {
...
test('grantTrackEndReward is idempotent per sceneKey (no double Master Ball / double gift)', () => {
    ST.sm = Object.assign({}, ST.sm, {
        balls: { poke:0, great:0, ultra:0, master:0 },
        inventory: {},
        _trackRewardGranted: {},
    });
    // Same boss key twice → one Master Ball; second call returns null.
    assert.ok(ST.grantTrackEndReward({ sceneKey: 'villain.rocket.boss' }));
    assert.equal(ST.grantTrackEndReward({ sceneKey: 'villain.rocket.boss' }), null);
    assert.equal(ST.sm.balls.master, 1);
    // Same raid key twice → 6 units once; second call returns null.
    assert.ok(ST.grantTrackEndReward({ sceneKey: 'extra.cubone.raid' }));
    assert.equal(ST.grantTrackEndReward({ sceneKey: 'extra.cubone.raid' }), null);
    assert.equal(ST.sm.inventory.expShareVoucher | 0, 6);
});
