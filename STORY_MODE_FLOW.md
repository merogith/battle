...
> **Anchor drift**: when a `battle.html:LINE` reference below no longer matches
> reality, resolve fresh via `node scripts/debug/symbol-index.mjs --lookup <symbol>`
> or browse `agent-state/ANCHOR_INDEX.md` (regenerated by the SessionStart hook
